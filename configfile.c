...
char		config_file_path[MAXUXPATH] = "";
...
load_config(const char *config_file, bool verbose, bool terse, char *argv0)
{
	...
	if (config_file != NULL && config_file[0] != '\0')
	{
		...
		if (config_file_path[0] != '/')
		{
			...
			if (pwd != NULL)
			{
				appendUXSQLExpBufferStr(&fullpath, pwd);
			}
			else
			{
				...
				if (getcwd(cwd, MAXUXPATH) == NULL)
				{
					log_error(_("unable to execute getcwd()"));
					log_detail("%s", strerror(errno));

					termUXSQLExpBuffer(&fullpath);
					exit(ERR_BAD_CONFIG);
				}

				appendUXSQLExpBufferStr(&fullpath, cwd);
			}

			appendUXSQLExpBuffer(&fullpath,
							  "/%s", config_file_path);

			log_debug("relative configuration file converted to:\n  \"%s\"",
					  fullpath.data);

			strncpy(config_file_path, fullpath.data, MAXUXPATH);
	 ...
	 * in this order:
	 *  - UXDB default conf directory:$UX_HOME/conf/ux_repmgr.conf
	 *  - current directory
	 *  - /etc/repmgr.conf
	 *  - default sysconfdir
	 *
	...
	if (config_file_provided == false)
	{
		...
		char		my_exec_path[MAXUXPATH] = "";
		char		home_path[MAXUXPATH] = "";

		/* 1. UXDB default sysconfdir */
		...
		*last_dir_separator(home_path) = '\0';
		...
		/* begin:Modify by douwen for #179457 #179454, 2023/3/30  reviewer:houyn*/
		else
		{
			maxpath_snprintf(config_file_path, "%s/conf/%s", home_path, CONFIG_FILE_NAME_HA);
			if (stat(config_file_path, &stat_config) == 0)
			{
				config_file_found = true;
				goto end_search;
		...
		/* end: Modify by douwen for #179457 #179454, 2023/3/30  reviewer:houyn*/
		/* 2 "./repmgr.conf" or "./.repmgr.conf" */
		...
		{
			maxpath_snprintf(config_file_path, "./%s", CONFIG_FILE_NAME_HA);
			canonicalize_path(config_file_path);
		...
		/* 3. "/etc/repmgr.conf"  or   "/etc/.repmgr.conf" */
		if (verbose == true)
		...
		{
			maxpath_snprintf(config_file_path, "/etc/%s", CONFIG_FILE_NAME_HA);
			if (stat(config_file_path, &stat_config) == 0)
			{
				config_file_found = true;
				goto end_search;
...
_parse_config(ItemList *error_list, ItemList *warning_list)
{
	...
	strncpy(base_directory, config_file_path, MAXUXPATH);
	...
	if (config_ok == true)
	{
		...
		else
		{
			...
			{
				UXSQLExpBufferData error_message_buf;
				initUXSQLExpBuffer(&error_message_buf);

				appendUXSQLExpBuffer(&error_message_buf,
								  _("\"conninfo\": %s	(provided: \"%s\")"),
								  conninfo_errmsg,
								  config_file_options.conninfo);

				item_list_append(error_list, error_message_buf.data);
				termUXSQLExpBuffer(&error_message_buf);
			}
		}
...
parse_configuration_item(ItemList *error_list, ItemList *warning_list, const char *name, const char *value)
{
	...
	do {
		if (strcmp(name, setting->name) == 0)
		{
			switch (setting->type)
			{
				...
				case CONFIG_INT:
				{
					if (strcmp(name, "repmgr_log_rotation_size") == 0 || strcmp(name, "repmgr_log_rotation_age") == 0)
						*(int *)setting->val.intptr = (*setting->process.process_func_p)(value, name);
					else
				...
				case CONFIG_STRING:
				{
					...
					if (strcmp(name, "wal_encparms_path") == 0)
					{
						canonicalize_path((char *)setting->val.strptr);
					}
	...
	else if (strcmp(name, "logfile") == 0 || strcmp(name, "log_file") == 0)
	{
		item_list_append(warning_list,
						 _("parameter \"logfile\" has been renamed to \"repmgr_log_filename\""));
	}
...
{
	char		recovery_conf_path[MAXUXPATH] = "";
	FILE	   *fp;
	char	   *s = NULL,
				buf[MAXLINELENGTH] = "";
	char		name[MAXLEN] = "";
	...
	snprintf(recovery_conf_path, MAXUXPATH,
			 "%s/%s",
			 data_dir,
...
reload_config(t_server_type server_type)
{
	...
	if (strncmp(config_file_options.conninfo, orig_config_file_options.conninfo, sizeof(config_file_options.conninfo)) != 0 && validate_conninfo_string(config_file_options.conninfo, NULL))
	{
		...
		if (!conn || (UXSQLstatus(conn) != CONNECTION_OK))
		{
			item_list_append_format(&config_errors,
	...
	if (config_errors.head != NULL)
	{
		...
		UXSQLExpBufferData errors;

		log_error(_("one or more errors encountered while parsing the configuration file"));

		initUXSQLExpBuffer(&errors);

		appendUXSQLExpBufferStr(&errors,
							 "following errors were detected:\n");

		for (cell = config_errors.head; cell; cell = cell->next)
		{
			appendUXSQLExpBuffer(&errors,
							  "  %s\n", cell->string);
		}

		log_detail("%s", errors.data);
		termUXSQLExpBuffer(&errors);
	...
	/* check_brain_split */
	if (config_file_options.check_brain_split != orig_config_file_options.check_brain_split)
	{
		item_list_append_format(&config_changes,
								_("\"check_brain_split\" changed from \"%s\" to \"%s\""),
								orig_config_file_options.check_brain_split ? "true" : "flase",
								config_file_options.check_brain_split ? "true" : "flase");
	}
	...
	/* repmgr_log_filename */
	if (strncmp(config_file_options.repmgr_log_filename, orig_config_file_options.repmgr_log_filename, sizeof(config_file_options.repmgr_log_filename)) != 0)
	{
		item_list_append_format(&config_changes,
								_("\"repmgr_log_filename\" changed from \"%s\" to \"%s\""),
								orig_config_file_options.repmgr_log_filename,
								config_file_options.repmgr_log_filename);
	}

	/* repmgr_log_directory */
	if (strncmp(config_file_options.repmgr_log_directory, orig_config_file_options.repmgr_log_directory, sizeof(config_file_options.repmgr_log_directory)) != 0)
	{
		item_list_append_format(&config_changes,
								_("\"repmgr_log_directory\" changed from \"%s\" to \"%s\""),
								orig_config_file_options.repmgr_log_directory,
								config_file_options.repmgr_log_directory);
	}

	/* try_synchronous_connection_timeout */
	if (config_file_options.try_synchronous_connection_timeout != orig_config_file_options.try_synchronous_connection_timeout)
	{
		item_list_append_format(&config_changes,
								_("\"try_synchronous_connection_timeout\" changed from \"%i\" to \"%i\""),
								orig_config_file_options.try_synchronous_connection_timeout,
								config_file_options.try_synchronous_connection_timeout);
	}

	/* root_password */
	if (strncmp(config_file_options.root_password, orig_config_file_options.root_password, sizeof(config_file_options.root_password)) != 0)
	{
		item_list_append_format(&config_changes,
								_("\"root_password\" changed from \"%s\" to \"%s\""),
								orig_config_file_options.root_password,
								config_file_options.root_password);
	}

	/* uxdb_password */
	if (strncmp(config_file_options.uxdb_password, orig_config_file_options.uxdb_password, sizeof(config_file_options.uxdb_password)) != 0)
	{
		item_list_append_format(&config_changes,
								_("\"uxdb_password\" changed from \"%s\" to \"%s\""),
								orig_config_file_options.uxdb_password,
	...
	if (config_changes.head != NULL)
	{
		...
		appendUXSQLExpBufferStr(&detail,
	...
	result = config_changes.head == NULL ? false : true;
	item_list_free(&config_errors);
	item_list_free(&config_changes);
	item_list_free(&config_warnings);
...
/* repmgr_rotation_time */
int
parse_time_interval(const char *value, const char *config_item)
{
	int i = 0;
	char str_num[MAXLEN] = {0};
	int num = 0;
	int num_unit = 0;

	for (i = 0; value[i] != 0 && i < MAXLEN; i ++)
	{
		if(isdigit(value[i]))
		{
			str_num[i] = value[i];
	...
	/* Determine whether the first character is an integer, or report an error if it is not */
	if(i == 0)
	{
		log_error(_("\"%s\" unable to interpret as unit"), config_item);
		exit(1);
	}
	...
	if(num < 1)
	{
		log_error(_("\"%s\" must range from 1 minute to 31 days!"), config_item);
		exit(1);
	}

	if(!strcasecmp(value + i, "d") || !strcasecmp(value + i, "day"))
	{
		num_unit = num * UINT_DAY;
	}
	else if(!strcasecmp(value + i, "h") || !strcasecmp(value + i, "hour"))
	{
		num_unit = num * UINT_HOUR;
	}
	else if(!strcasecmp(value + i, "m") || !strcasecmp(value + i, "min"))
	{
		num_unit = num * UINT_MINUTE;
	}
	else
	{
		log_error(_("\"%s\" unable to interpret as unit"), config_item);
...
/* repmgr_rotation_size */
int
parse_size(const char *value, const char *config_item)
{
	int i = 0;
	char str_num[MAXLEN] = {0};
	int num = 0;
	...
	if(num < 1)
	{
		log_error(_("\"%s\" must range from 1 kb to 1024 MB!"), config_item);
		exit(1);
	}

	if(!strcasecmp(value + i, "MB"))
	{
		num_unit = num * UINT_MB;
	}
	else if(!strcasecmp(value + i, "KB"))
	{
		num_unit = num * UINT_KB ;
	}
	else
	{
		log_error(_("\"%s\" unable to interpret as unit"), config_item);
 ...
 *
 * TODO: accept "any unambiguous prefix of one of these" as per uxsinodb.conf:
 *
 *   https://www.uxsino.com/docs/current/config-setting.html
 */
bool
...
tablespace_list_append(TablespaceList *tablespace_mapping, const char *arg)
{
	...
	cell = (TablespaceListCell *) ux_malloc0(sizeof(TablespaceListCell));
	if (cell == NULL)
	{
...
tablespace_list_copy(t_configuration_options *original, t_configuration_options *copy)
{
	...
	{
		TablespaceListCell *copy_cell = (TablespaceListCell *) ux_malloc0(sizeof(TablespaceListCell));
...
{
	UXSQLExpBufferData auto_conf;
	UXSQLExpBufferData auto_conf_tmp;
	UXSQLExpBufferData auto_conf_contents;
	...
	initUXSQLExpBuffer(&auto_conf);
	appendUXSQLExpBuffer(&auto_conf, "%s/%s",
			data_dir, UX_AUTOCONF_FILENAME);

	success = ProcessUxdbConfigFile(auto_conf.data,
										NULL,
	...
	initUXSQLExpBuffer(&auto_conf_tmp);
	appendUXSQLExpBuffer(&auto_conf_tmp, "%s.tmp",
			auto_conf.data);

	initUXSQLExpBuffer(&auto_conf_contents);

	/*
	 * Keep this in sync with src/backend/utils/misc/guc.c:write_auto_conf_file()
	 */
	appendUXSQLExpBufferStr(&auto_conf_contents,
			"# Do not edit this file manually!\n"
			"# It will be overwritten by the ALTER SYSTEM command.\n");

	for (cell = config.head; cell; cell = cell->next)
	{
		appendUXSQLExpBuffer(&auto_conf_contents,
				"%s = '%s'\n",
				cell->key, cell->value);
	 ...
	 * directory. In UxsinoDB 11 and later this can be 0700 or 0750.
	 */
	...
	else
	{
		...
		else
		{
			...
			/*
			 * Note: durable_rename() is not exposed to frontend code before Ux 10.
			 * We only really need to be modifying uxsinodb.auto.conf from Ux 12,
			 * but provide backwards compatibility for Ux 9.6 and earlier for the
			 * (unlikely) event that a repmgr built against one of those versions
			 * is being used against Ux 12 and later.
			 */
	...
	termUXSQLExpBuffer(&auto_conf);
	termUXSQLExpBuffer(&auto_conf_tmp);
	termUXSQLExpBuffer(&auto_conf_contents);
...
parse_event_notifications_list(EventNotificationList *event_notifications, const char *arg)
{
	...
	for (arg_ptr = arg; arg_ptr <= (arg + strlen(arg)); arg_ptr++)
	{
		...
		if ((*arg_ptr == ',' || *arg_ptr == '\0') && event_type_buf[0] != '\0')
		{
			...
			cell = (EventNotificationListCell *) ux_malloc0(sizeof(EventNotificationListCell));
...
parse_output_to_argv(const char *string, char ***argv_array)
{
	...
	options_string = ux_malloc0(options_len);
	 ...
	 * program name)
	 */
	local_argv_array = ux_malloc0(sizeof(char *) * (argc_item + 2));

	/* Insert a blank dummy program name at the start of the array */
	local_argv_array[0] = ux_malloc0(1);
	...
	for (cell = option_argv.head; cell; cell = cell->next)
	{
		...
		local_argv_array[c] = (char *)ux_malloc0(argv_len);
...
bool
parse_ux_basebackup_options(const char *ux_basebackup_options, t_basebackup_options *backup_options, int server_version_num, ItemList *error_list)
{
	...
	if (!strlen(ux_basebackup_options))
	...
	argc_item = parse_output_to_argv(ux_basebackup_options, &argv_array);
							...
							&optindex)) != -1)
	{
		switch (c)
		{
			...
			case 'X':
				...
				break;
			case 1:
				strncpy(backup_options->waldir, optarg, MAXUXPATH);
				break;
			case 2:
...
{
	UXSQLExpBufferData buf;
	char *ptr;
	EventNotificationListCell *cell;
	int ptr_len;

	initUXSQLExpBuffer(&buf);
	cell = list->head;

	while (cell != NULL)
	{
		appendUXSQLExpBufferStr(&buf, cell->event_type);
