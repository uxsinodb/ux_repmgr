/*
 * controldata.c - functions for reading the ux_control file
 *
 * The functions provided here enable repmgr to read a ux_control file
 * in a version-indepent way, even if the UXsinoDB instance is not
 * running. For that reason we can't use on the ux_control_*() functions
 * provided in UXsinoDB 9.6 and later.
 *
...
int
get_ux_version(const char *data_directory, char *version_string)
{
	char		UxVersionPath[MAXUXPATH] = "";
	FILE	   *fp = NULL;
	char	   *endptr = NULL;
	...
	snprintf(UxVersionPath, MAXUXPATH, "%s/UX_VERSION", data_directory);

	fp = fopen(UxVersionPath, "r");
	...
	{
		log_warning(_("unable to determine major version number from UX_VERSION"));
...
get_controlfile(const char *DataDir)
{
	...
	int			fd, version_num;
	char		ControlFilePath[MAXUXPATH] = "";
	...
	/*
	 * Read UX_VERSION, as we'll need to determine which struct to read
	 * the control file contents into
	 */

	version_num = get_ux_version(DataDir, file_version_string);

	if (version_num == UNKNOWN_SERVER_VERSION_NUM)
	{
		log_warning(_("unable to determine server version number from UX_VERSION"));
	...
	if (version_num < MIN_SUPPORTED_VERSION_NUM)
	{
		log_warning(_("data directory appears to be initialised for %s"),
					file_version_string);
		log_detail(_("minimum supported UXsinoDB version is %s"),
				   MIN_SUPPORTED_VERSION);
		return control_file_info;
	}

	snprintf(ControlFilePath, MAXUXPATH, "%s/global/ux_control", DataDir);

	if ((fd = open(ControlFilePath, O_RDONLY | UX_BINARY, 0)) == -1)
	{
		log_warning(_("could not open file \"%s\" for reading"),
					ControlFilePath);
		log_detail("%s", strerror(errno));
	...
	/*
	 * We don't check the CRC here as we're potentially checking a ux_control
	 * file from a different UXsinoDB version to the one repmgr was compiled
	 * against.
	 */
