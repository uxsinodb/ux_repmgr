...
do_primary_register(void)
{
	...
	char    virtual_ip[MAXLEN] = ""; //uxdb
	char    network_card[MAXLEN] = ""; //uxdb
	bool    old_vip_found = false; //uxdb

	UXSQLExpBufferData event_description;
	...
	initUXSQLExpBuffer(&event_description);
	...
	if (record_status == RECORD_FOUND)
	{
		...
		{
			appendUXSQLExpBufferStr(&event_description,
								 "existing primary record updated");
		}
		else
		{
			appendUXSQLExpBuffer(&event_description,
	...
	termUXSQLExpBuffer(&event_description);
	...
	if (record_status == RECORD_FOUND)
	{
		/* uxdb: Force register primary, unbind the old virtual ip */
		if (old_vip_found)
		{
			unbind_virtual_ip(virtual_ip, network_card, config_file_options.uxdb_password);
		}
	...
	{
		/* uxdb: When primary node has registerd successful, bind the virtual ip to node's network card*/
		if(check_vip_conf(config_file_options.virtual_ip, config_file_options.network_card))
		{
			bind_virtual_ip(config_file_options.virtual_ip, config_file_options.network_card, config_file_options.uxdb_password);
		}
...
do_primary_unregister(void)
{
	...
	if (UXSQLstatus(target_node_conn) != CONNECTION_OK)
	{
