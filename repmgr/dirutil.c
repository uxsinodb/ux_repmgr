...
/* NB: uxdb_fe must be included BEFORE check_dir */
...
{
	char create_dir_path[MAXUXPATH];

	/* mkdir_p() may modify the supplied path */
	strncpy(create_dir_path, path, MAXUXPATH);
...
bool
is_ux_dir(const char *path)
{
	char		dirpath[MAXUXPATH] = "";
	struct stat sb;

	/* test uxdata */
	snprintf(dirpath, MAXUXPATH, "%s/UX_VERSION", path);
...
/*
 * Attempt to determine if a UXsinoDB data directory is in use
...
is_ux_running(const char *path)
{
	...
	snprintf(pid_file, MAXUXPATH, "%s/uxmaster.pid", path);

	pidf = fopen(pid_file, "r");
	if (pidf == NULL)
	{
		/*
		 * No PID file - UXsinoDB shouldn't be running. From 9.3 (the
		 * earliest version we care about) removal of the PID file will
		 * cause the uxmaster to shut down, so it's highly unlikely
		 * that UXsinoDB will still be running.
		 */
		if (errno == ENOENT)
		{
			return UX_DIR_NOT_RUNNING;
		}
		else
		{
			log_error(_("unable to open UXsinoDB PID file \"%s\""), pid_file);
			log_detail("%s", strerror(errno));
			exit(ERR_BAD_CONFIG);
		}
	}
	 ...
	 * as UXsinoDB should have shut itself down in these cases anyway.
	 */
	if (fscanf(pidf, "%ld", &pid) != 1)
	{
		...
		{
			log_warning(_("UXsinoDB PID file \"%s\" is empty"), path);
		}
		else
		{
			log_warning(_("invalid data in UXsinoDB PID file \"%s\""), path);
		}
...
bool
create_ux_dir(const char *path, bool force)
{
	/* Check this directory can be used as a UXDATA dir */
