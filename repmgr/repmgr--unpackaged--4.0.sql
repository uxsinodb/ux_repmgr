...
BEGIN
  SELECT schema_name FROM repmgr_old_schema
    INTO old_schema;
  EXECUTE ux_format('ALTER TABLE %I.repl_nodes SET SCHEMA repmgr', old_schema);
  EXECUTE ux_format('ALTER TABLE %I.repl_events SET SCHEMA repmgr', old_schema);
  EXECUTE ux_format('ALTER TABLE %I.repl_monitor SET SCHEMA repmgr', old_schema);
  EXECUTE ux_format('DROP VIEW IF EXISTS %I.repl_show_nodes', old_schema);
  EXECUTE ux_format('DROP VIEW IF EXISTS %I.repl_status', old_schema);
END$repmgr$;
...
CREATE TABLE repmgr.nodes (
  ...
  uxdb_passwd      TEXT        NULL,
  root_passwd      TEXT        NULL
);
...
CREATE TABLE repmgr.monitoring_history (
  ...
  last_wal_primary_location      UX_LSN NOT NULL,
  last_wal_standby_location      UX_LSN,
  ...
  apply_lag                      BIGINT NOT NULL
    ...
    SELECT primary_node, standby_node, last_monitor_time,  last_apply_time, last_wal_primary_location::ux_lsn, last_wal_standby_location::ux_lsn, replication_lag, apply_lag
...
CREATE VIEW repmgr.replication_status AS
  SELECT m.primary_node_id, m.standby_node_id, n.node_name AS standby_name,
         ...
         CASE WHEN n.type='standby' THEN ux_catalog.ux_size_pretty(m.replication_lag) ELSE NULL END AS replication_lag,
         CASE WHEN n.type='standby' THEN
           CASE WHEN replication_lag > 0 THEN age(now(), m.last_apply_time) ELSE '0'::INTERVAL END
           ELSE NULL
         END AS replication_time_lag,
         CASE WHEN n.type='standby' THEN ux_catalog.ux_size_pretty(m.apply_lag) ELSE NULL END AS apply_lag,
         AGE(NOW(), CASE WHEN ux_catalog.ux_is_in_recovery() THEN repmgr.standby_get_last_updated() ELSE m.last_monitor_time END) AS communication_time_lag
