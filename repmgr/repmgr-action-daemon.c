...
{
	UXconn	   *conn = NULL;
	UXSQLExpBufferData repmgrd_command;
	...
	{
		/* TODO: if UXsinoDB is not available, have repmgrd loop and retry connection */
		log_error(_("unable to connect to local node"));
		log_detail(_("UXsinoDB must be running before \"repmgrd\" can be started"));
		exit(ERR_DB_CONN);
	}
	...
	initUXSQLExpBuffer(&repmgrd_command);
	appendUXSQLExpBufferStr(&repmgrd_command,
	...
	termUXSQLExpBuffer(&repmgrd_command);
	...
	/* BEGIN:  Added by li_menglin for #208475, 2025/1/2  reviewer:huyns */
	conn = establish_db_connection(config_file_options.conninfo, false);
	if(conn != NULL)
	{
			...
			&& check_vip_conf(config_file_options.virtual_ip, config_file_options.network_card))
		{
			...
			{
				log_notice(_("need bind the virtual ip if virtual don't bind when start primary server"));
			}
		}
	}
	UXSQLfinish(conn);
	/* END:  Added by li_menglin for #208475, 2025/1/2  reviewer:huyns */
}
...
void do_daemon_stop(void)
{
	...
	{
		/*
		 * a UXsinoDB connection is not required to stop repmgrd,
		 */
