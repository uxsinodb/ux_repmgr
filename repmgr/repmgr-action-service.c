...
static void fetch_node_records(UXconn *conn, NodeInfoList *node_list);
...
do_service_status(void)
{
	...
	repmgrd_info = (RepmgrdInfo **) ux_malloc0(sizeof(RepmgrdInfo *) * nodes.node_count);
	...
	strncpy(headers_status[STATUS_UX].title, _("Status"), MAXLEN);
	...
	for (cell = nodes.head; cell; cell = cell->next)
	{
		...
		snprintf(repmgrd_info[i]->ux_running_text, sizeof(cell->node_info->details),
				 "%s", node_status.data);
		...
		headers_status[STATUS_UX].cur_length = strlen(repmgrd_info[i]->ux_running_text);
	...
	for (cell = nodes.head; cell; cell = cell->next)
	{
		if (runtime_options.output_mode == OM_CSV)
		{
			...
			/* If UxsinoDB is not running, repmgrd status is unknown */
		...
		else
		{
			...
			printf("| %-*s ", headers_status[STATUS_UX].max_length, repmgrd_info[i]->ux_running_text);
...
void
fetch_node_records(UXconn *conn, NodeInfoList *node_list)
{
