...
static UXconn *source_conn = NULL;

static char local_data_directory[MAXUXPATH] = "";
...
static standy_clone_mode mode = ux_basebackup;

/* used by barman mode */
static char local_repmgr_tmp_directory[MAXUXPATH] = "";
...
static void _do_standby_promote_internal(UXconn *conn);
...
static bool check_upstream_config(UXconn *conn, int server_version_num, t_node_info *node_info, bool exit_on_error);
static void check_primary_standby_version_match(UXconn *conn, UXconn *primary_conn);
static void check_recovery_type(UXconn *conn);
...
static int	run_ux_backupapi(t_node_info *node_record);
...
static void write_primary_conninfo(UXSQLExpBufferData *dest, t_conninfo_param_list *param_list);
 ...
 *  --verify-backup (UxsinoDB 13 and later)
 */
...
do_standby_clone(void)
{
	 ...
	 * use the (mandatory) value from that; if -D/--uxdata was provided, use
	 * that.
	 *
	 * Note that barman mode requires -D/--uxdata.
	 */
	...
	else
	{
		...
		log_hint(_("use -D/--uxdata to explicitly specify a data directory"));
		exit(ERR_BAD_CONFIG);
	}
	...
	switch (check_dir(local_data_directory))
	{
		...
		case DIR_NOT_EMPTY:
			...
			if (is_ux_dir(local_data_directory))
			{
				/* even -F/--force is not enough to overwrite an active directory... */
				if (is_ux_running(local_data_directory))
				{
					log_error(_("specified data directory \"%s\" appears to contain a running UXsinoDB instance"),
							  local_data_directory);
					log_hint(_("ensure the target data directory does not contain a running UXsinoDB instance"));
					exit(ERR_BAD_CONFIG);
				}
			}
			break;
	...
	if (runtime_options.no_upstream_connection == false)
	{
		...
		{
			/*
			 * --verify-backup available for UxsinoDB 13 and later
			 */
			if (UXSQLserverVersion(source_conn) < 130000)
			{
				log_error(_("--verify-backup available for UxsinoDB 13 and later"));
				exit(ERR_BAD_CONFIG);
			}
		}
	...
	else if (mode == barman && UXSQLstatus(source_conn) != CONNECTION_OK)
	{
		/*
	...
	if (recovery_conninfo_str[0] == '\0')
	{
		...
		if (UXSQLstatus(source_conn) == CONNECTION_OK)
			UXSQLfinish(source_conn);
		exit(ERR_BAD_CONFIG);
	}
	 ...
	 */
	if (UXSQLstatus(source_conn) == CONNECTION_OK && runtime_options.copy_external_config_files == true)
	{
		UXconn	   *superuser_conn = NULL;
		UXconn	   *privileged_conn = NULL;
		 ...
		 * data directory, they won't be copied by ux_basebackup, but we can't
	...
	if (runtime_options.dry_run == true)
	{
		 ...
		 */
		if (config_file_options.use_replication_slots == true && UXSQLstatus(source_conn) == CONNECTION_OK)
		{
			UXSQLExpBufferData msg;
			bool success = true;
		...
		if (mode == ux_basebackup && runtime_options.fast_checkpoint == false)
		{
			log_hint(_("consider using the -c/--fast-checkpoint option"));
		}

		if (mode == ux_basebackup)
		{
			/*
			 * In --dry-run mode, this will just output the ux_basebackup command which
			 * would be executed.
			 */
	...
	switch (mode)
	{
		case ux_basebackup:
			...
			log_notice(_("starting backup (using ux_basebackup)..."));
			break;
		case barman:
			log_notice(_("retrieving backup from Barman..."));
			break;
		case ux_backupapi:
			log_notice(_("starting backup (using ux_backupapi)..."));
			break;
		default:
	...
	switch (mode)
	{
		...
		case barman:
			...
			break;
		case ux_backupapi:
			r = run_ux_backupapi(&local_node_record);
			break;
		default:
			/* should never reach here */
			log_error(_("unknown clone mode"));
	...
	/*
	 * Run ux_verifybackup here if requested, before any alterations are made
	 * to the data directory.
	 */
	if (mode == ux_basebackup && runtime_options.verify_backup == true)
	{
		...
		make_ux_path(&command, "ux_verifybackup");
		...
		appendUXSQLExpBufferChar(&command, ' ');

		/* Somewhat inconsistent, but ux_verifybackup doesn't accept a -D option  */
	...
	if (runtime_options.copy_external_config_files == true && config_files.entries > 0)
	{
		 ...
		 * have been copied.
		 */
		if (runtime_options.copy_external_config_files_destination == CONFIG_FILE_UXDATA)
			copy_configuration_files(false);
	}
	...
	{
		case ux_basebackup:
			log_notice(_("standby clone (using ux_basebackup) complete"));
			break;
		...
		case ux_backupapi:
			log_notice(_("standby clone (from ux_backupapi) complete"));
			break;
	}
	 ...
	 * is Uxdb10 or earlier, Uxdb will refuse to start. We might not have
	 ...
	 * UXsinoDB server automatically
	 */

	log_notice(_("you can now start your UXsinoDB server"));
	...
	{
		log_hint(_("for example: ux_ctl -D %s start"),
				 local_data_directory);
	}
	else
	{
		log_hint(_("for example: /etc/init.d/uxsinodb start"));
	}
	...
	{
		case ux_basebackup:
			appendUXSQLExpBufferStr(&event_details, "ux_basebackup");
			break;
		case barman:
			appendUXSQLExpBufferStr(&event_details, "barman");
			break;
		case ux_backupapi:
			appendUXSQLExpBufferStr(&event_details, "ux_backupapi");
			break;
	}
	...
	if (primary_conn != source_conn && UXSQLstatus(primary_conn) == CONNECTION_OK)
	{
	...
	/* begin:Modify by liwei for #191709, 2023/10/13  reviewer:houjx */
	/*删除.cur_system_time文件*/
	cur_system_time_file = strcat(config_file_options.data_directory, "/.cur_system_time");

	if(remove(cur_system_time_file) != 0)
	{
		log_warning("Failed to delete %s file.", cur_system_time_file);
	}
	/* end:Modify by liwei for #191709, 2023/10/13  reviewer:houjx */
...
{
	UXSQLExpBufferData command;
	UXSQLExpBufferData	command_shell;
	...
	initUXSQLExpBuffer(&command);
	initUXSQLExpBuffer(&command_shell);

	/* modify by houjiaxing for #178952 at 2023/03/16 reveiwer huyuanni. */
	if (!config_file_options.uxdb_password)
	{
		appendUXSQLExpBuffer(&command, "%s show-backup %s latest > /dev/null",
	...
	{
		appendUXSQLExpBuffer(&command, "%s",
						make_barman_ssh_command(barman_command_buf));
		appendUXSQLExpBuffer(&command_shell, "show-backup %s latest > /dev/null",
						config_file_options.barman_server);

		command_ok = local_command_sshpass(command.data, command_shell.data, NULL, config_file_options.uxdb_password);
	}
	...
	if (command_ok == false)
	{
		log_error(_("no valid backup for server \"%s\" was found in the Barman catalogue"),
				  config_file_options.barman_server);
		log_detail(_("command executed was:\n  %s"), !config_file_options.uxdb_password ? command.data : command_shell.data),
		log_hint(_("refer to the Barman documentation for more information"));

		termUXSQLExpBuffer(&command);
		termUXSQLExpBuffer(&command_shell);
		exit(ERR_BARMAN);
	}
	...
	if (runtime_options.dry_run == false)
	{
		if (!create_ux_dir(local_data_directory, runtime_options.force))
		{
		...
		if (!create_ux_dir(local_repmgr_tmp_directory, runtime_options.force))
		{
	...
	if (!config_file_options.uxdb_password)
	{
		if (runtime_options.dry_run == true)
		{
			appendUXSQLExpBuffer(&command, "%s show-server %s > /dev/null",
							  make_barman_ssh_command(barman_command_buf),
							  config_file_options.barman_server);
		}
		else
		{
			appendUXSQLExpBuffer(&command, "%s show-server %s > %s/show-server.txt",
							  make_barman_ssh_command(barman_command_buf),
							  config_file_options.barman_server,
							  local_repmgr_tmp_directory);
		}
	...
	else
	{
		...
		if (runtime_options.dry_run == true)
		{
			appendUXSQLExpBuffer(&command_shell, "%s show-server %s > /dev/null",
					make_barman_ssh_command(barman_command_buf),
					config_file_options.barman_server);
		}
		else
		{
			appendUXSQLExpBuffer(&command_shell, "%s show-server %s > %s/show-server.txt",
					make_barman_ssh_command(barman_command_buf),
					config_file_options.barman_server,
					local_repmgr_tmp_directory);
		}
...
_do_create_replication_conf(void)
{
	...
	RecordStatus record_status = RECORD_NOT_FOUND;
	char		recovery_file_path[MAXUXPATH + sizeof(RECOVERY_COMMAND_FILE)] = "";
	struct stat st;
	bool		node_is_running = false;
	bool		slot_creation_required = false;
	UXconn	   *upstream_conn = NULL;
	UXconn	   *upstream_repl_conn = NULL;
	...
	{
		log_error(_("no data directory provided"));
		log_hint(_("provide the node's \"repmgr.conf\" file with -f/--config-file or the data directory with -D/--uxdata"));
		exit(ERR_BAD_CONFIG);
	}
	...
	switch (check_dir(local_data_directory))
	{
		...
		case DIR_NOT_EMPTY:
			...
			if (!is_ux_dir(local_data_directory))
			{
				log_error(_("specified data directory \"%s\" does not contain a UXsinoDB instance"), local_data_directory);
				UXSQLfinish(source_conn);
				exit(ERR_BAD_CONFIG);
			}
			...
			if (is_ux_running(local_data_directory))
			{
				if (runtime_options.force == false)
				{
					...
					if (UXSQLserverVersion(source_conn) >= 120000)
					{
	...
	if (record_status != RECORD_FOUND)
	{
		...
		{
			log_detail("%s", UXSQLerrorMessage(source_conn));
		}
	...
	if (UXSQLstatus(upstream_repl_conn) != CONNECTION_OK)
	{
		if (runtime_options.force == false)
		{
	...
	/* check if recovery.conf exists (Ux11 and earlier only) */
	if (UXSQLserverVersion(upstream_conn) < 120000)
	{
	...
	if (runtime_options.dry_run == true)
	{
		...
		create_recovery_file(&local_node_record,
							 &recovery_conninfo,
							 UXSQLserverVersion(upstream_conn),
							 recovery_conf_contents,
							 false);

		if (UXSQLserverVersion(upstream_conn) >= 120000)
		{
			log_info(_("following items would be added to \"uxsinodb.auto.conf\" in \"%s\""), local_data_directory);
		}
		else
		{
	...
	else
	{
								  ...
								  true))
		{
			if (UXSQLserverVersion(upstream_conn) >= 120000)
			{
				log_error(_("unable to write replication configuration to \"uxsinodb.auto.conf\""));
			}
			else
			{
		...
		else
		{
			if (UXSQLserverVersion(upstream_conn) >= 120000)
			{
				log_notice(_("replication configuration written to \"uxsinodb.auto.conf\""));
			}
			else
			{
			...
			{
				if (UXSQLserverVersion(upstream_conn) >= 130000)
				{
					log_hint(_("configuration must be reloaded for the configuration changes to take effect"));
				}
				else if (UXSQLserverVersion(upstream_conn) >= 120000)
				{
	...
	/* Ux12 and later: add standby.signal, if not already there */
	if (UXSQLserverVersion(upstream_conn) >= 120000)
	{
		if (runtime_options.dry_run == true)
		{
...
do_standby_register(void)
{
	 ...
	 */
	if (UXSQLstatus(conn) != CONNECTION_OK && runtime_options.force == false)
	{
		bool		conn_ok = false;
		int			timer = 0;
			...
			else
			{
				...
				else
				{
					if (!runtime_options.force)
					{
						...
						log_detail(_("no record for application name \"%s\" found in \"ux_stat_replication\""),
								   config_file_options.node_name);
						log_hint(_("use -F/--force to continue anyway"));
						UXSQLfinish(primary_conn);
						if (UXSQLstatus(conn) == CONNECTION_OK)
	 ...
	 */
	if (runtime_options.upstream_node_id == NO_UPSTREAM_NODE && UXSQLstatus(conn) == CONNECTION_OK)
	{
		...
		if (record_status != RECORD_FOUND)
		{
			...
			else if (runtime_options.force == false)
			{
				...
				/* TODO: 9.6 and later, display detail from ux_stat_wal_receiver */
...
do_standby_promote(void)
{
	...
	UXconn	   *current_primary_conn = NULL;
	...
	/*
	 * In UxsinoDB 12 and earlier, executing "ux_ctl ... promote" when WAL
	 ...
	 */
	if (UXSQLserverVersion(local_conn) < 130000)
	{
		...
		if (replay_paused == true)
		{
			if (UXSQLserverVersion(local_conn) >= 100000)
				log_hint(_("execute \"ux_wal_replay_resume()\" to unpause WAL replay"));
			else
				log_hint(_("execute \"ux_xlog_replay_resume()\" to unpause WAL replay"));
	...
	if (UXSQLstatus(current_primary_conn) == CONNECTION_OK)
	{
		...
		UXSQLfinish(current_primary_conn);
		UXSQLfinish(local_conn);
		exit(ERR_PROMOTION_FAIL);
	}
	else if (runtime_options.dry_run == true)
	 ...
	 * For Uxdb12 and later, check whether ux_promote() can be executed.
	 */
	if (runtime_options.dry_run == true)
	{
		...
		else if (UXSQLserverVersion(local_conn) >= 120000)
		{
			if (can_execute_ux_promote(local_conn) == false)
			{
				log_info(_("node will be promoted using \"ux_ctl promote\""));
				log_detail(_("user \"%s\" does not have permission to execute \"ux_promote()\""),
						   UXSQLuser(local_conn));
			}
			else
			{
				log_info(_("node will be promoted using the \"ux_promote()\" function"));
			}
		}
		else
		{
	...
	/*
	 * uxdb: after promoting local server to new primary, add the vip to local node
	 */
...
static void
_do_standby_promote_internal(UXconn *conn)
{
	int			i;
	 ...
	 *
	 * "ux_ctl promote" returns immediately and (prior to 10.0) has no -w
	 ...
	 *
	 * For UXsinoDB 12+, use the ux_promote() function, unless one of
	 * "service_promote_command" or "use_ux_ctl_promote" is set.
	 */
	{
		bool use_ux_promote = false;
		...
		if (UXSQLserverVersion(conn) >= 120000)
		{
			...
			if (config_file_options.service_promote_command[0] != '\0')
			{
				use_ux_promote = false;
			}
			else if (can_execute_ux_promote(conn) == false)
			{
				use_ux_promote = false;
				log_info(_("user \"%s\" does not have permission to execute \"ux_promote()\", falling back to \"ux_ctl promote\""),
						 UXSQLuser(conn));
			}
		}
		...
		if (use_ux_promote == true)
		{
			log_detail(_("promoting server \"%s\" (ID: %i) using ux_promote()"),
					   local_node_record.node_name,
					   local_node_record.node_id);
	...
	if (promote_success == false)
	{
		if (recovery_type == RECTYPE_STANDBY)
		{
			...
			log_hint(_("the node may need more time to promote itself, check the UXsinoDB log for details"));
			UXSQLfinish(conn);
			exit(ERR_PROMOTION_FAIL);
		}
		else
...
do_standby_follow(void)
{
	...
	UXconn	   *follow_target_conn = NULL;
	...
	for (timer = 0; timer < config_file_options.primary_follow_timeout; timer++)
	{
		...
		if (UXSQLstatus(follow_target_conn) == CONNECTION_OK || runtime_options.wait_provided == false)
		{
			break;
		}
		sleep(1);
	...
	if (UXSQLstatus(follow_target_conn) != CONNECTION_OK)
	{
		if (follow_target_node_id == UNKNOWN_NODE_ID)
		{
	...
	if (runtime_options.upstream_node_id == UNKNOWN_NODE_ID)
	{
		...
		if (record_status != RECORD_FOUND)
		{
			log_error(_("unable to find record for follow target node %i"),
					  follow_target_node_id);
			UXSQLfinish(follow_target_conn);
			exit(ERR_FOLLOW_FAIL);
		}
	}
	 ...
	 */
	{
		UXSQLExpBufferData node_info_msg;
		RecoveryType recovery_type = RECTYPE_UNKNOWN;
		initUXSQLExpBuffer(&node_info_msg);
		...
		if (recovery_type == RECTYPE_PRIMARY)
		{
			...
			appendUXSQLExpBuffer(&node_info_msg,
		...
		termUXSQLExpBuffer(&node_info_msg);
	}
	...
	/* check if we can attach to the follow target */
	{
		 ...
		 * ux_control.
		 *
		 * TODO: from 9.6, query "ux_stat_wal_receiver" via the existing local connection
		 */
	 ...
	 * upstream.
	 * For 9.6 and later, we could check ux_stat_wal_receiver on the local node.
	 */
	...
	if (success == true)
	{
		...
		appendUXSQLExpBuffer(&follow_output,
 ...
 */
bool
do_standby_follow_internal(UXconn *primary_conn, UXconn *follow_target_conn, t_node_info *follow_target_node_record, UXSQLExpBufferData *output, int general_error_code, int *error_code)
{
	t_node_info local_node_record = T_NODE_INFO_INITIALIZER;
	...
	if (!create_recovery_file(&local_node_record,
							  &recovery_conninfo,
							  UXSQLserverVersion(primary_conn),
...

	{
		...
		if (server_up == true)
		{
			...
			if (UXSQLserverVersion(primary_conn) >= 130000 && config_file_options.standby_follow_restart == false)
			{
				/* UxsinoDB 13 and later: we'll send SIGHUP via ux_ctl */
			...
			{
				/* no "service_restart_command" defined - stop and start using ux_ctl*/
...
cleanup:
	...
	if (remove_old_replication_slot == true)
	{
		...
		{
			UXconn	   *old_upstream_conn = establish_db_connection_quiet(original_upstream_node_record.conninfo);

			if (UXSQLstatus(old_upstream_conn) != CONNECTION_OK)
			{
...
do_standby_switchover(void)
{
	...
	RecordStatus record_status = RECORD_NOT_FOUND;
	RecoveryType recovery_type = RECTYPE_UNKNOWN;
	UXSQLExpBufferData remote_command_str;
	UXSQLExpBufferData command_output;
	UXSQLExpBufferData node_rejoin_options;
	UXSQLExpBufferData logmsg;
	UXSQLExpBufferData detailmsg;
	UXSQLExpBufferData event_details;
	...
	char		node_rejoin_log[MAXUXPATH] = "";

	NodeInfoList sibling_nodes = T_NODE_INFO_LIST_INITIALIZER;
	SiblingNodeStats sibling_nodes_stats = T_SIBLING_NODES_STATS_INITIALIZER;

	/* this will be calculated as max_wal_senders - COUNT(*) FROM ux_stat_replication */
	int			available_wal_senders = 0;

	/* uxdb: virtual ip setting variable */
	char    virtual_ip[MAXLEN] = "";
	char    network_card[MAXLEN] = "";
	char    unbind_vip[MAXLEN] = "";
	char    unbind_vip_shell[MAXLEN] = "";
	 ...
	 * directory owner.
	 *
	 * For UxsinoDB 11 and earlier, if UxsinoDB is not able to rename "recovery.conf",
	 * promotion will fail.
	 *
	 * For UxsinoDB 12 and later, promotion will not fail even if "uxsinodb.auto.conf"
	 * is owned by another user, but we'll check just in case, as it is indicative of a
	 * poorly configured setup. In any case we will need to check "uxsinodb.auto.conf" on
	 * the demotion candidate as the rejoin will fail if we are unable to to write to that.
	 */

	initUXSQLExpBuffer(&logmsg);
	initUXSQLExpBuffer(&detailmsg);

	if (check_replication_config_owner(UXSQLserverVersion(local_conn),
									   config_file_options.data_directory,
									   &logmsg, &detailmsg) == false)
	{
		...
		termUXSQLExpBuffer(&logmsg);
		termUXSQLExpBuffer(&detailmsg);
	...
	/*
	 * In UxsinoDB 12 and earlier, check that WAL replay on the standby
	...
	if (UXSQLserverVersion(local_conn) < 130000 && is_wal_replay_paused(local_conn, false) == true)
	{
		ReplInfo 	replication_info;
		init_replication_info(&replication_info);
	...
	/*
	 * If --force-rewind specified, check ux_rewind can be used, and
	...
	{
		UXSQLExpBufferData reason;
		UXSQLExpBufferData msg;

		initUXSQLExpBuffer(&reason);

		if (can_use_ux_rewind(remote_conn, config_file_options.data_directory, &reason) == false)
		{
			log_error(_("--force-rewind specified but ux_rewind cannot be used"));
			log_detail("%s", reason.data);
			termUXSQLExpBuffer(&reason);
			UXSQLfinish(local_conn);
			UXSQLfinish(remote_conn);
	...
	initUXSQLExpBuffer(&remote_command_str);
	 ...
	 * next step.
	 */
	appendUXSQLExpBufferStr(&remote_command_str, "--version >/dev/null 2>&1 && echo \"1\" || echo \"0\"");
	...
	command_success = remote_command(remote_host,
									 ...
									 &command_output,
									 remote_node_record.uxdb_passwd);

	termUXSQLExpBuffer(&remote_command_str);

	if (command_success == false || command_output.data[0] == '0')
	{
		UXSQLExpBufferData hint;
		...
		initUXSQLExpBuffer(&hint);
		appendUXSQLExpBufferStr(&hint,
							 _("check \"ux_bindir\" is set to the correct path in \"repmgr.conf\"; current value: "));

		if (strlen(config_file_options.ux_bindir))
		{
			appendUXSQLExpBuffer(&hint,
							  "\"%s\"", config_file_options.ux_bindir);
		}
		else
		{
			appendUXSQLExpBufferStr(&hint,
	...
	appendUXSQLExpBufferStr(&remote_command_str, "--version 2>/dev/null");
	initUXSQLExpBuffer(&command_output);
	command_success = remote_command(remote_host,
									 runtime_options.remote_user,
									 remote_command_str.data,
	...
	appendUXSQLExpBuffer(&remote_command_str,
	 ...
	 * anyway.
	 */
	appendUXSQLExpBufferStr(&remote_command_str, "node check --data-directory-config --optformat -LINFO 2>/dev/null");
	...
	/* check remote repmgr has the data directory correctly configured */
	{
		...
		if (parse_data_directory_config(command_output.data, &remote_error) == false)
		{
			if (remote_error != REMOTE_ERROR_NONE)
			{
				...
				{
					UXSQLExpBufferData ssh_command;

					/* can happen if the connection configuration is not consistent across nodes */
					log_detail(_("an error was encountered when attempting to connect to UxsinoDB on node \"%s\" (ID: %i)"),
							   remote_node_record.node_name,
							   remote_node_record.node_id);
					...
					appendUXSQLExpBufferStr(&remote_command_str, "node check --db-connection");

					initUXSQLExpBuffer(&ssh_command);
					...
					termUXSQLExpBuffer(&remote_command_str);
					termUXSQLExpBuffer(&ssh_command);
				}
				...
				{
					log_detail(_("an unknown error was encountered when attempting to connect to UxsinoDB on node \"%s\" (ID: %i)"),
							   remote_node_record.node_name,
							   remote_node_record.node_id);
				}
			}
	...
	/*
	 * For UxsinoDB 12 and later, check "uxsinodb.auto.conf" is owned by the
	...
	if (UXSQLserverVersion(local_conn) >= 120000 && remote_repmgr_version >= 50100)
	{
		initUXSQLExpBuffer(&remote_command_str);
		make_remote_repmgr_path(&remote_command_str, &remote_node_record);

		appendUXSQLExpBufferStr(&remote_command_str, "node check --replication-config-owner --optformat -LINFO 2>/dev/null");
	...
	/* check archive/replication status */
	{
		...
		if (guc_set(remote_conn, "archive_mode", "!=", "off"))
		{
			...
			initUXSQLExpBuffer(&remote_command_str);
			make_remote_repmgr_path(&remote_command_str, &remote_node_record);
			appendUXSQLExpBufferStr(&remote_command_str,
				...
				case CHECK_STATUS_CRITICAL:
					{
						if (runtime_options.force == false)
						{
							...
							log_detail(_("%i pending archive files (critical threshold: %i)"),
									   files, threshold);
							log_hint(_("UXsinoDB will not shut down until all files are archived; use -F/--force to continue anyway"));
							UXSQLfinish(remote_conn);
							UXSQLfinish(local_conn);
				...
				case CHECK_STATUS_WARNING:
					{
						...
						log_detail(_("%i pending archive files (warning threshold: %i)"),
								   files, threshold);
						log_hint(_("UXsinoDB will not shut down until all files are archived"));
					}
					break;
		...
		if (lag_seconds >= config_file_options.replication_lag_critical)
		{
			if (runtime_options.force == false)
			{
				...
				log_hint(_("UXsinoDB on the demotion candidate will not shut down until pending WAL is flushed to the standby; use -F/--force to continue anyway"));
				UXSQLfinish(remote_conn);
				UXSQLfinish(local_conn);
	...
	if (runtime_options.repmgrd_no_pause == false)
	{
		...
		for (cell = all_nodes.head; cell; cell = cell->next)
		{
			...
			if (UXSQLstatus(cell->node_info->conn) != CONNECTION_OK)
			{
				...
				repmgrd_info[i]->ux_running = false;
				...
				if (cell->node_info->active == true)
				{
					...
					item_list_append_format(&repmgrd_connection_errors,
											...
											UXSQLerrorMessage(cell->node_info->conn));
				}
		...
		if (unreachable_node_count > 0)
		{
			...
			if (runtime_options.force == false)
			{
				...
				/*
				 * 程序退出之前，对分配的内存进行释放。不确定这些对象是否真的
				 * 需要被释放，为了规范对它们进行了内存释放。不会出现非法释放问题。
				 */
				if (repmgrd_info != NULL)
				{
					NodeInfoListCell *ncell = NULL;

					i = 0;
					for (ncell = all_nodes.head; ncell; ncell = ncell->next)
					{
						ux_free(repmgrd_info[i]);
						repmgrd_info[i] = NULL;
						i++;
					}
					ux_free(repmgrd_info);
					repmgrd_info = NULL;
				}
		...
		if (repmgrd_running_count > 0)
		{
			...
			for (cell = all_nodes.head; cell; cell = cell->next)
			{
				 ...
				 */
				if (repmgrd_info[i]->ux_running == false)
				{
				...
				{
					log_notice(_("repmgrd not running on node \"%s\" (ID: %i)"),
								cell->node_info->node_name,
								cell->node_info->node_id);
					i++;
					continue;
	 ...
	 *
	 * We'll issue the ux_ctl command but not force it not to wait; we'll
	...
	for (i = 0; i < config_file_options.shutdown_check_timeout; i++)
	{
		...
		ping_res = UXSQLping(remote_conninfo);

		log_debug("ping status is: %s", print_uxsqlping_status(ping_res));

		/* database server could not be contacted */
		if (ping_res == UXSQLPING_NO_RESPONSE || ping_res == UXSQLPING_NO_ATTEMPT)
		{
	...
	/*
	 * uxdb: after stopping remote primary server, delete the vip of primary node
	 * tianbing
	 */
	if (get_virtual_ip(local_conn, remote_node_id, virtual_ip) && get_network_card(local_conn, remote_node_id, network_card))
	{
		...
		{
			/* add by houjiaxing for #178952 at 2023/03/16 reveiwer huyuanni. */
			if(!strlen(remote_node_record.root_passwd))
			{
				sprintf(unbind_vip, "ssh %s \"sudo ip addr del %s dev %s\"", remote_host, virtual_ip, network_card);
				ux_system(unbind_vip);
			}
			else
			{
		...
		{
			log_warning(_("standby_switchover:configure virtual_ip: %s or network_card: %s is invaild"), virtual_ip, network_card);
			/* BEGIN Added by chen_jingwen for #207866 at 2025/01/14 */
	 ...
	 *
	 * If UxsinoDB 12 or later, and -S/--superuser provided, we will provide
	 * a superuser connection so that ux_promote() can be used.
	 */

	if (UXSQLserverVersion(local_conn) >= 120000 && superuser_conn != NULL)
	{
	...
	/*
	 * If ux_rewind is requested, issue a checkpoint immediately after promoting
	 * the local node, as ux_rewind compares timelines on the basis of the value
	 * in ux_control, which is written at the first checkpoint, which might not
	 * occur immediately.
	 */
	if (runtime_options.force_rewind_used == true)
	{
		UXconn *checkpoint_conn = local_conn;
		if (superuser_conn != NULL)
		{
	 ...
	 * remote server. Additionally execute "ux_rewind", if required and
	 * requested.
	 */
	initUXSQLExpBuffer(&node_rejoin_options);

	/*
	 * Don't wait for repmgr on the remote node to report the success
	 * of the rejoin operation - we'll check it from here.
	 */
	appendUXSQLExpBufferStr(&node_rejoin_options,
						 " --no-wait");
	...
	if (replication_info.last_wal_receive_lsn < remote_last_checkpoint_lsn)
	{
		...
		if (runtime_options.force_rewind_used == false)
		{
			...
			termUXSQLExpBuffer(&node_rejoin_options);
			UXSQLfinish(local_conn);
			/* BEGIN Added by chen_jingwen for #207866 at 2024/10/8 */
			/*
			 * 程序退出之前，对分配的内存进行释放。不确定这些对象是否真的
		...
		{
			appendUXSQLExpBuffer(&node_rejoin_options,
							  "=%s",
							  runtime_options.force_rewind_path);
		}

/* #201574 此次修正仅针对linux平台,在ux_rewind过程中规避配置文件的覆盖问题,因此不再需要 --config-files 参数的指定
 * 注:当然目前版本的 repmgr 是不支持windows的,如果未来支持windows,此 WIN32 宏定义就是有意义的
 */
#ifdef WIN32
		appendUXSQLExpBufferStr(&node_rejoin_options,
							 " --config-files=");
		...
		for (cell = remote_config_files.head; cell; cell = cell->next)
		{
			if (first_entry == false)
				appendUXSQLExpBufferChar(&node_rejoin_options, ',');
			else
				first_entry = false;

			appendUXSQLExpBufferStr(&node_rejoin_options, cell->key);
		}
#endif
		appendUXSQLExpBufferChar(&node_rejoin_options, ' ');
	}
	...
	/* */
	snprintf(node_rejoin_log, MAXUXPATH,
	...
	else
	{
		...
		if (command_output.data[0] == '0')
		{
			...
			if (UXSQLstatus(remote_conn) == CONNECTION_OK)
			{
				...
				{
					appendUXSQLExpBuffer(&detailmsg,
									  _("UxsinoDB instance on demotion candidate \"%s\" (ID: %i) is running as a primary\n"),
									  remote_node_record.node_name,
									  remote_node_record.node_id);
		...
		else
		{
			...
			{
				appendUXSQLExpBuffer(&detailmsg,
								  "check the UxsinoDB log file on  demotion candidate \"%s\" (ID: %i)",
								  remote_node_record.node_name,
								  remote_node_record.node_id);
			}
		}
	...
	appendUXSQLExpBufferStr(&event_details, logmsg.data);
	...
	if (runtime_options.repmgrd_no_pause == false)
	{
		if (repmgrd_running_count > 0)
		{
			...
			/* Added by chen_jingwen for #207866 at 2024/10/8 */
			item_list_free(&repmgrd_unpause_errors);
		}
...
check_source_server()
{
	...
	if (extension_status != REPMGR_INSTALLED)
	{
		if (!runtime_options.force)
		{
			...
			if (extension_status == REPMGR_UNKNOWN)
			{
				...
				log_detail("%s", UXSQLerrorMessage(primary_conn));
				UXSQLfinish(source_conn);
				exit(ERR_DB_QUERY);
			}
	...
	else
	{
		 ...
		 *
		 * This check will be only carried out on UxsinoDB 9.6 and
		...
		if (get_recovery_type(source_conn) == RECTYPE_PRIMARY && UXSQLserverVersion(source_conn) >= 90600)
		{
	...
	/*
	 * Check the local directory to see if it appears to be a UxsinoDB
	 * data directory.
	 *
	 * Note: a previous call to check_dir() will have checked whether it contains
	 * a running UxsinoDB instance.
	 */
	if (is_ux_dir(local_data_directory))
	{
		const char *msg = _("target data directory appears to be a UxsinoDB data directory");
	...
	if (upstream_node_id != UNKNOWN_NODE_ID)
	{
		...
		if (record_status == RECORD_FOUND)
		{
			...
			upstream_conninfo_found = true;
			/* Added by chen_jingwen for #207866 at 2024/10/8 */
			free_conninfo_params(&upstream_conninfo);
		}
	...
	{
		/*
		 * This will check if the user is superuser or (from Uxdb10) is a member
		 * of "ux_read_all_settings"/"ux_monitor"
		 */
		if (connection_has_ux_monitor_role(source_conn, "ux_read_all_settings") == true)
		{
		...
		else
		{
			...
			if (UXSQLserverVersion(source_conn) >= 100000)
			{
				log_hint(_("the repmgr user must be superuser or member of role \"ux_monitor\" or \"ux_read_all_settings\", or a superuser provided with -S/--superuser"));
			}
			else
			{
...
check_source_server_via_barman()
{
	...
	UXSQLExpBufferData command_output;
	UXSQLExpBufferData repmgr_conninfo_buf;
	...
	initUXSQLExpBuffer(&repmgr_conninfo_buf);

	for (c = 0; c < barman_conninfo.size && barman_conninfo.keywords[c] != NULL; c++)
	{
		if (repmgr_conninfo_buf.len != 0)
			appendUXSQLExpBufferChar(&repmgr_conninfo_buf, ' ');

		appendUXSQLExpBuffer(&repmgr_conninfo_buf, "%s=",
	...
	{
		maxlen_snprintf(buf,
					"ssh -o StrictHostKeyChecking=no uxdb@%s",
					config_file_options.barman_host);
		maxlen_snprintf(buf_shell,
					" uxsql -Aqt \"%s\" -c \""
					" SELECT conninfo"
					" FROM repmgr.nodes"
					" WHERE %s"
...
static bool
check_upstream_config(UXconn *conn, int server_version_num, t_node_info *upstream_node_record, bool exit_on_error)
{
	int			i;
	bool		config_ok = true;
	...
	/*
	 * Parse "ux_basebackup_options", if set, to detect whether --wal-method
	 * has been set to something other than `stream` (i.e. `fetch`), as this
	 * will influence some checks
	 */

	backup_options_ok = parse_ux_basebackup_options(config_file_options.ux_basebackup_options,
	...
	if (backup_options_ok == false)
	{
		...
		{
			log_error(_("error(s) encountered parsing \"ux_basebackup_options\""));
			print_error_list(&backup_option_errors, LOG_ERR);
			log_hint(_("\"ux_basebackup_options\" is: \"%s\""),
					 config_file_options.ux_basebackup_options);
			exit(ERR_BAD_CONFIG);
		}
	...
	if (config_file_options.use_replication_slots == true)
	{
		ux_setting_ok = get_ux_setting_int(conn, "max_replication_slots", &i);

		if (ux_setting_ok == false || i < 1)
		{
			if (ux_setting_ok == true)
			{
		...
		if (ux_setting_ok == true && i > 0 && runtime_options.dry_run == true)
		{
	...
	else if (mode != barman)
	{
		 ...
		 * required if ux_basebackup is being used with --wal-method=fetch,
		...
		if (check_wal_keep_segments == true)
		{
			...
			if (UXSQLserverVersion(conn) < 130000)
				wal_keep_parameter_name = "wal_keep_segments";

			ux_setting_ok = get_ux_setting_int(conn, wal_keep_parameter_name, &i);
			...
			if (ux_setting_ok == false || i < 1)
			{
				if (ux_setting_ok == true)
				{
					...
					log_hint(_("Choose a value sufficiently high enough to retain enough WAL "
							   ...
							   "Alternatively set up WAL archiving using e.g. UxBarman and configure "
							   "'restore_command' in repmgr.conf to fetch WALs from there."));
					log_hint(_("In UxsinoDB 9.4 and later, replication slots can be used, which "
...
*/
	...
	ux_setting_ok = get_ux_setting_int(conn, "max_wal_senders", &i);
	...
	else if (ux_setting_ok == true && i > 0 && runtime_options.dry_run == true)
	{
		log_info(_("parameter \"max_wal_senders\" set to %i"), i);
	}

	/*
	 * If using ux_basebackup, ensure sufficient replication connections can
	 * be made. There's no guarantee they'll still be available by the time
	 * ux_basebackup is executed, but there's nothing we can do about that.
	...
	if (mode == ux_basebackup)
	{
		...
		connections = ux_malloc0(sizeof(UXconn *) * min_replication_connections);
...

	{
		...
		/* data_checksums available from UxsinoDB 9.3; can be read by any user */
		if (get_ux_setting_bool(conn, "data_checksums", &data_checksums) == false)
		{
		...
		/* wal_log_hints available from UxsinoDB 9.4; can be read by any user */
		if (get_ux_setting_bool(conn, "wal_log_hints", &wal_log_hints) == false)
		{
			/* highly unlikely this will happen */
		...
		if (data_checksums == false && wal_log_hints == false)
		{
			...
			log_detail(_("ux_rewind requires \"wal_log_hints\" to be enabled"));
		}
	}
 ...
 *
 * In ux_basebackup mode, configure the target data directory
...
initialise_direct_clone(t_node_info *local_node_record, t_node_info *upstream_node_record)
{
	 ...
	 *
	 * ux_basebackup doesn't verify mappings, so any errors will not be
	 ...
	 * node; this must be done before ux_basebackup is called.
	 *
	 ...
	 *
	 * TODO: skip this for Ux10, and ensure temp slot option used
	 *
...
{
	UXSQLExpBufferData params;
	UXSQLExpBufferData script;
	...
	/*
	 * Parse the ux_basebackup_options provided in repmgr.conf - we'll want to
	 * check later whether certain options were set by the user
	 */
	parse_ux_basebackup_options(config_file_options.ux_basebackup_options,
								&backup_options,
								source_server_version_num,
								NULL);

	/* Create ux_basebackup command line options */

	initUXSQLExpBuffer(&params);

	appendUXSQLExpBuffer(&params, " -D %s", local_data_directory);

	/*
	 * conninfo string provided - pass it to ux_basebackup as the -d option
	 * (ux_basebackup doesn't require or want a database name, but for
	...
	if (runtime_options.conninfo_provided == true)
	{
		...
		appendUXSQLExpBuffer(&params, " -d '%s'", conninfo_str);
	 ...
	 * them individually to ux_basebackup (-d/--dbname not required)
	 */
	else
	{
		if (strlen(runtime_options.host))
		{
			appendUXSQLExpBuffer(&params, " -h %s", runtime_options.host);
		}

		if (strlen(runtime_options.port))
		{
			appendUXSQLExpBuffer(&params, " -p %s", runtime_options.port);
		}

		if (strlen(runtime_options.replication_user))
		{
			appendUXSQLExpBuffer(&params, " -U %s", runtime_options.replication_user);
		}
		else if (strlen(upstream_repluser))
		{
			appendUXSQLExpBuffer(&params, " -U %s", upstream_repluser);
		}
		else if (strlen(node_record->repluser))
		{
			appendUXSQLExpBuffer(&params, " -U %s", node_record->repluser);
		}
		else if (strlen(runtime_options.username))
		{
			appendUXSQLExpBuffer(&params, " -U %s", runtime_options.username);
		}
	...
	{
		appendUXSQLExpBufferStr(&params, " -c fast");
	}
	...
	if (config_file_options.tablespace_mapping.head != NULL)
	{
		for (cell = config_file_options.tablespace_mapping.head; cell; cell = cell->next)
		{
			appendUXSQLExpBuffer(&params, " -T %s=%s", cell->old_dir, cell->new_dir);
		}
	}
	...
	{
		appendUXSQLExpBufferStr(&params, " -X stream");
	}

	/*
	 * From 9.6, ux_basebackup accepts -S/--slot, which forces WAL streaming
	 ...
	 *
	 * NOTE: It's possible to set 'ux_basebackup_options' with an invalid
	...
	if (source_server_version_num >= 90600 && config_file_options.use_replication_slots)
	{
		...
		/*
		 * Check whether 'ux_basebackup_options' in repmgr.conf has the --slot
		...
		if (slot_add == true)
		{
			appendUXSQLExpBuffer(&params, " -S %s", node_record->slot_name);
		}
	}

	initUXSQLExpBuffer(&script);
	make_ux_path(&script, "ux_basebackup");

	appendUXSQLExpBuffer(&script,
					  " -l \"repmgr base backup\" %s %s",
					  params.data,
					  config_file_options.ux_basebackup_options);

	termUXSQLExpBuffer(&params);

	if (runtime_options.dry_run == true)
	{
		log_info(_("would execute:\n  %s"), script.data);
		termUXSQLExpBuffer(&script);
		return SUCCESS;
	}
	...
	/*
	 * As of 9.4, ux_basebackup only ever returns 0 or 1
	 */

	r = ux_system(script.data);
...
run_file_backup(t_node_info *local_node_record)
{
	...
	UXSQLExpBufferData tablespace_map;
	...
	if (mode == barman)
	{
		...
		{
			initUXSQLExpBuffer(&tablespace_map);
		}
		...
		if (source_server_version_num == UNKNOWN_SERVER_VERSION_NUM)
		{
			...
			source_server_version_num = get_ux_version(local_data_directory, NULL);
		...
		/*
		 * Parse the ux_basebackup_options provided in repmgr.conf - we need to
		...
		/*
		 * We must create some UXDATA subdirectories because they are not
		 ...
		 */
		{
			const char *const dirs[] = {
				...
				"ux_commit_ts",
				/* Only from 9.4 */
				"ux_dynshmem", "ux_logical", "ux_logical/snapshots", "ux_logical/mappings", "ux_replslot",
				/* Present in all versions from  9.3 */
				"ux_notify", "ux_serial", "ux_snapshots", "ux_stat", "ux_stat_tmp",
				"ux_subtrans", "ux_tblspc", "ux_twophase",
			...
			for (i = 0; dirs[i]; i++)
			{
				...
				/*
				 * If --waldir/--xlogdir specified in "ux_basebackup_options",
				 * create a symlink rather than make a directory.
				 */
				if (strcmp(dirs[i], "ux_wal") == 0 || strcmp(dirs[i], "ux_xlog") == 0)
				{
					if (backup_options.waldir[0] != '\0')
					{
						if (create_ux_dir(backup_options.waldir, false) == false)
						{
							/* create_ux_dir() will log specifics */
	...
	for (cell_t = tablespace_list.head; cell_t; cell_t = cell_t->next)
	{
		...
		if (mode == barman)
		{
			create_ux_dir(tblspc_dir_dest, false);
		 ...
		 * will be copied as-is by ux_basebackup and no action is required)
		 */
		if (mapping_found == true || mode == barman)
		{
			...
			if (source_server_version_num >= 90500)
			{
				...
				appendUXSQLExpBuffer(&tablespace_map,
								  "%s %s\n",
								  cell_t->oid,
								  tblspc_dir_dest);
			}
			...
			/*
			 * Pre-9.5, we have to manipulate the symlinks in ux_tblspc/
			 * ourselves
			 */
			else
			{
				UXSQLExpBufferData tblspc_symlink;

				initUXSQLExpBuffer(&tblspc_symlink);
				appendUXSQLExpBuffer(&tblspc_symlink, "%s/ux_tblspc/%s",
								  local_data_directory,
								  cell_t->oid);
	...
	{
		UXSQLExpBufferData tablespace_map_filename;
		FILE	   *tablespace_map_file;

		initUXSQLExpBuffer(&tablespace_map_filename);
		appendUXSQLExpBuffer(&tablespace_map_filename, "%s/%s",
						  local_data_directory,
						  TABLESPACE_MAP);
		...
		termUXSQLExpBuffer(&tablespace_map_filename);
		termUXSQLExpBuffer(&tablespace_map);
	}
...
/*
 * Perform a call to ux_backupapi endpoint to ask barman to write the backup
 ...
 */
static int
run_ux_backupapi(t_node_info *local_node_record)
{
	int r = ERR_UXBACKUPAPI_SERVICE;
	...
	check_ux_backupapi_standby_clone_options();

	task->host = malloc(strlen(config_file_options.ux_backupapi_host)+1);
	task->remote_ssh_command = malloc(strlen(config_file_options.ux_backupapi_remote_ssh_command)+1);
	task->node_name = malloc(strlen(config_file_options.ux_backupapi_node_name)+1);
	task->operation_type = malloc(strlen(DEFAULT_STANDBY_UX_BACKUPAPI_OP_TYPE)+1);
	task->backup_id = malloc(strlen(config_file_options.ux_backupapi_backup_id)+1);
	...
	strcpy(task->host, config_file_options.ux_backupapi_host);
	strcpy(task->remote_ssh_command, config_file_options.ux_backupapi_remote_ssh_command);
	strcpy(task->node_name, config_file_options.ux_backupapi_node_name);
	strcpy(task->operation_type, DEFAULT_STANDBY_UX_BACKUPAPI_OP_TYPE);
	strcpy(task->backup_id, config_file_options.ux_backupapi_backup_id);
	...
	if ((ret != CURLE_OK) || (strlen(task->operation_id) == 0)) {
		...
		if (499 > http_return_code && http_return_code >= 400) {
			...
			log_error("whilst reaching out ux_backup service: %s\n", curl_easy_strerror(ret));
		}
		}
	else
	{
...
/*
 * ux_backupapi mode is enabled when config_file_options.ux_backupapi_host is set hence, we
 * should also check the other required variables too.
 */

void check_ux_backupapi_standby_clone_options() {

	bool error = false;

	if (*config_file_options.ux_backupapi_remote_ssh_command == '\0') {
		log_hint("Check config: remote ssh command is required");
		error = true;
	}
	if (*config_file_options.ux_backupapi_node_name == '\0') {
		log_hint("Check config: node name is required");
		error = true;
	}
	if (*config_file_options.ux_backupapi_backup_id == '\0') {
...
{
	/*
	 * Example: [('main', 24674, '/var/lib/uxsinodb/tablespaces/9.5/main'),
	 * ('alt', 24678, '/var/lib/uxsinodb/tablespaces/9.5/alt')]
	 */

	char		name[MAXLEN] = "";
	char		oid[MAXLEN] = "";
	char		location[MAXUXPATH] = "";
...
copy_configuration_files(bool delete_after_copy)
{
	...
	for (i = 0; i < config_files.entries; i++)
	{
		...
		initUXSQLExpBuffer(&dest_path);

		if (runtime_options.copy_external_config_files_destination == CONFIG_FILE_SAMEPATH)
		{
			appendUXSQLExpBufferStr(&dest_path, file->filepath);
		}
		else
		{
			appendUXSQLExpBuffer(&dest_path,
							  "%s/%s",
							  local_data_directory,
							  file->filename);
		}
...
tablespace_data_append(TablespaceDataList *list, const char *name, const char *oid, const char *location)
{
	...
	cell = (TablespaceDataListCell *) ux_malloc0(sizeof(TablespaceDataListCell));
	...
	cell->oid = ux_malloc0(1 + oid_len);
	cell->name = ux_malloc0(1 + name_len);
	cell->location = ux_malloc0(1 + location_len);
 ...
 */
static void
check_primary_standby_version_match(UXconn *conn, UXconn *primary_conn)
{
	...
	if ((primary_version_num / 100) != (standby_version_num / 100))
	{
		UXSQLfinish(conn);
		UXSQLfinish(primary_conn);
		log_error(_("UXsinoDB versions on primary (%s) and standby (%s) must match"),
...
static void
check_recovery_type(UXconn *conn)
{
...
{
	UXSQLExpBufferData recovery_file_buf;
	UXSQLExpBufferData primary_conninfo_buf;
	char		recovery_file_path[MAXUXPATH] = "";
	FILE	   *recovery_file;
	mode_t		um;
	...
	initUXSQLExpBuffer(&primary_conninfo_buf);

	/* standby_mode = 'on' (Ux 11 and earlier) */
	 ...
	 *
	 * UxsinoDB 11 and earlier only; 'latest' is the default from UxsinoDB 12.
	 */
	...
	if (as_file == false)
	{
		...
		initUXSQLExpBuffer(&recovery_file_buf);

		for (cell = recovery_config.head; cell; cell = cell->next)
		{
			appendUXSQLExpBuffer(&recovery_file_buf,
							  "%s = '%s'\n",
							  cell->key, cell->value);
		}
		...
		termUXSQLExpBuffer(&recovery_file_buf);
		/* BEGIN Added by chen_jingwen for #207866 at 2024/10/8 */
		termUXSQLExpBuffer(&primary_conninfo_buf);
		key_value_list_free(&recovery_config);
		/* END Added by chen_jingwen for #207866 at 2024/10/8 */
	...
	/*
	 * UxsinoDB 12 and later: modify uxsinodb.auto.conf
	 */
	if (server_version_num >= 120000)
	{
	...
	/*
	 * UxsinoDB 11 and earlier: write recovery.conf
	 */
...
static void
write_primary_conninfo(UXSQLExpBufferData *dest, t_conninfo_param_list *param_list)
{
	...
	for (c = 0; c < param_list->size && param_list->keywords[c] != NULL; c++)
	{
		...
		appendUXSQLExpBuffer(&conninfo_buf, "%s=", param_list->keywords[c]);
	...
	if (application_name_provided == false)
	{
		...
		{
			appendUXSQLExpBufferStr(&conninfo_buf, " application_name=");
			appendConnStrVal(&conninfo_buf, config_file_options.node_name);
		}
		else
		{
			appendUXSQLExpBufferStr(&conninfo_buf, " application_name=repmgr");
		}
	}
	...
	if (password_provided == false)
	{
		if (config_file_options.use_primary_conninfo_password == true)
		{
			...
			if (password != NULL)
			{
				appendUXSQLExpBufferStr(&conninfo_buf, " password=");
	...
	{
		/* check if the libuxsql we're using supports "passfile=" */
		if (has_passfile() == true)
		{
			appendUXSQLExpBufferStr(&conninfo_buf, " passfile=");
	...
	appendUXSQLExpBufferStr(dest, escaped);
...
check_sibling_nodes(NodeInfoList *sibling_nodes, SiblingNodeStats *sibling_nodes_stats)
{
	...
	if (runtime_options.siblings_follow == false)
	{
		...
		{
			UXSQLExpBufferData nodes;
			NodeInfoListCell *cell;

			initUXSQLExpBuffer(&nodes);

			for (cell = sibling_nodes->head; cell; cell = cell->next)
			{
				appendUXSQLExpBuffer(&nodes,
								  "  %s (node ID: %i",
								  cell->node_info->node_name,
								  cell->node_info->node_id);
				...
				if (cell->node_info->type == WITNESS)
				{
					appendUXSQLExpBufferStr(&nodes,
										 ", witness server");
				}
				appendUXSQLExpBufferChar(&nodes,
									  ')');
				if (cell->next)
					appendUXSQLExpBufferStr(&nodes, "\n");
			}
...
sibling_nodes_follow(t_node_info *local_node_record, NodeInfoList *sibling_nodes, SiblingNodeStats *sibling_nodes_stats)
{
	...
	/*
	 * TODO: double-check all expected nodes are in _stat_replication
...
do_standby_help(void)
{
	...
	puts("");
	/* BEGIN:  Added by huyn for #159877, 2022/9/22  reviewer:houjiaxing */
	...
	printf(_("  --copy-external-config-files[={samepath|uxdata}]\n" \
	...
	printf(_("  --upstream-conninfo                 \"primary_conninfo\" value to include in the recovery configuration \n" \
			 "                                        when the intended upstream server does not yet exist\n"
			 "                                        note please exexute --recovery-conf-only ,then can get primary_conninfo value\n"));
	printf(_("  --upstream-node-id                  ID of the upstream node to replicate from (optional, defaults to primary node)\n"));
#if (UX_VERSION_NUM >= 130000)
	printf(_("  --verify-backup                     verify a cloned node using the \"ux_verifybackup\" utility\n"));
#endif
	...
	printf(_("  --recovery-min-apply-delay          set UxsinoDB configuration parameter \"recovery_min_apply_delay\"\n" \
	...
	printf(_("  -F, --force                         ignore warnings and continue anyway\n"));
	printf(_("  --force-rewind[=VALUE]              use \"ux_rewind\" to reintegrate the old primary if necessary\n"));
	printf(_("                                        (UxsinoDB 9.4 - provide \"ux_rewind\" path)\n"));
