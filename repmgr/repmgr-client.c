...
#include <storage/fd.h>			/* for UX_TEMP_FILE_PREFIX */
...
static void _determine_replication_slot_user(UXconn *conn,
											 t_node_info *upstream_node_record,
											 char **replication_user);

static UXconn *_get_replication_slot_connection(UXconn *conn,
												char *replication_user,
...
main(int argc, char **argv)
{
	 ...
	 *
	 * Note: UXSQLconndefaults() does not provide a default value for "dbname",
	 ...
	 * would break the libuxsql behaviour if non-default username is provided.
	 */
							...
							&optindex)) != -1)
	{
		...
		switch (c)
		{
			...
			case 'V':
				...
				/* #206829 解析repmgr版本号失败，因此switchover失败;考虑到现场repmgr版本不一定会对应，因此恢复到原生单独的版本号 */
				/* BEGIN:  Added by haoxg for #95098, 2020/12/2  reviewer:gouqq */
				printf("%s %s\n", progname(), REPMGR_VERSION);
				/* printf("%s (%s %s)\n", progname(), UX_PROGRAM_NAME, UX_PROMPT_VERSION); */
				/* END:  Added by haoxg for #95098, 2020/12/2  reviewer:gouqq */
				/* Added by chen_jingwen for #207866 at 2024/10/8 */
				free_conninfo_params(&source_conninfo);
				exit(SUCCESS);
			...
			/* END:  Added by haoxg for #95098, 2020/12/2  reviewer:gouqq */
				...
				/* -b/--ux_bindir */
			case 'b':
				strncpy(runtime_options.ux_bindir, optarg, MAXLEN);
				break;
			...
			case 'p':
				/*
				 * minimum TCP port number is 1; in practice UXsinoDB
			...
			case 'S':
				...
				/* -D/--uxdata/--data-dir */
			case 'D':
				strncpy(runtime_options.data_dir, optarg, MAXUXPATH);
				break;
			...
			case 'c':
				...
				/* --copy-external-config-files(=[samepath|uxdata]) */
			case OPT_COPY_EXTERNAL_CONFIG_FILES:
				...
				if (optarg != NULL)
				{
					...
					/* allow "data_directory" as synonym for "uxdata" */
					else if (strcmp(optarg, "uxdata") == 0 || strcmp(optarg, "data_directory") == 0)
					{
						runtime_options.copy_external_config_files_destination = CONFIG_FILE_UXDATA;
					}
					else
					{
						item_list_append(&cli_errors,
										 _("value provided for \"--copy-external-config-files\" must be \"samepath\" or \"uxdata\""));
					}
				}
				break;
			...
			case OPT_FORCE_REWIND:
				...
				if (optarg != NULL)
				{
					strncpy(runtime_options.force_rewind_path, optarg, MAXUXPATH);
				}
			...
			case OPT_CONFIG_ARCHIVE_DIR:
				...
				strncpy(runtime_options.config_archive_dir, optarg, MAXUXPATH);
				break;
			...
			case 'L':
				{
					...
					{
						UXSQLExpBufferData invalid_log_level;

						initUXSQLExpBuffer(&invalid_log_level);
						appendUXSQLExpBuffer(&invalid_log_level, _("invalid log level \"%s\" provided"), optarg);
						item_list_append(&cli_errors, invalid_log_level.data);
						termUXSQLExpBuffer(&invalid_log_level);
					}
					break;
				}
	...
	{
		if (strncmp(runtime_options.dbname, "uxsinodb://", 13) == 0 ||
			strncmp(runtime_options.dbname, "uxdb://", 11) == 0 ||
			strchr(runtime_options.dbname, '=') != NULL)
		{
			char	   *errmsg = NULL;
			UXSQLconninfoOption *opts;

			runtime_options.conninfo_provided = true;

			opts = UXSQLconninfoParse(runtime_options.dbname, &errmsg);

			if (opts == NULL)
			{
				UXSQLExpBufferData conninfo_error;

				initUXSQLExpBuffer(&conninfo_error);
				appendUXSQLExpBuffer(&conninfo_error, _("error parsing conninfo:\n%s"), errmsg);
				item_list_append(&cli_errors, conninfo_error.data);

				termUXSQLExpBuffer(&conninfo_error);
				pfree(errmsg);
			}
			else
			{
	...
	if (repmgr_command != NULL)
	{
		...
		else if (strcasecmp(repmgr_command, "NODE") == 0)
		{
			...
			if (strcasecmp(repmgr_action, "CHECK") == 0)
				...
				action = NODE_CONTROL;
			else if (strcasecmp(repmgr_action, "STARTUP") == 0)	//uxdb
				action = NODE_STARTUP;
		}
	...
	if (action == NO_ACTION)
	{
		UXSQLExpBufferData command_error;

		initUXSQLExpBuffer(&command_error);

		if (repmgr_command == NULL)
		{
			appendUXSQLExpBuffer(&command_error,
	...
	if (action == STANDBY_CLONE)
	{
		if (optind < argc)
		{
			...
			{
				UXSQLExpBufferData additional_host_arg;

				initUXSQLExpBuffer(&additional_host_arg);
				appendUXSQLExpBuffer(&additional_host_arg,
	...
	if (optind < argc)
	{
		UXSQLExpBufferData too_many_args;

		initUXSQLExpBuffer(&too_many_args);
		appendUXSQLExpBuffer(&too_many_args, _("too many command-line arguments (first extra is \"%s\")"), argv[optind]);
	...
	/*
	 * Initialise ux_bindir - command line parameter will override any setting
	 * in the configuration file
	 */
	if (!strlen(runtime_options.ux_bindir))
	{
		strncpy(runtime_options.ux_bindir, config_file_options.ux_bindir, MAXLEN);
	}

	/* Add trailing slash */
	if (strlen(runtime_options.ux_bindir))
	{
		int			len = strlen(runtime_options.ux_bindir);

		if (runtime_options.ux_bindir[len - 1] != '/')
		{
			maxlen_snprintf(ux_bindir, "%s/", runtime_options.ux_bindir);
		}
		else
		{
			strncpy(ux_bindir, runtime_options.ux_bindir, MAXLEN);
		}
	}
...
check_cli_parameters(const int action)
{
		...
		case STANDBY_CLONE:
			{
				...
				else
				{
					...
					/*
					 * If -D/--uxdata was provided, but config_file_options.uxdata
					 * is set, warn that -D/--uxdata will be ignored.
					 */
					if (runtime_options.data_dir[0] && config_file_options.data_directory[0])
					{
						item_list_append(&cli_warnings,
										 _("-D/--uxdata will be ignored if a repmgr configuration file is provided"));
	...
	if (runtime_options.data_dir[0])
	{
		switch (action)
		{
			...
			default:
				item_list_append_format(&cli_warnings,
										_("-D/--uxdata not required when executing %s"),
										action_name(action));
		}
	}
 ...
 */
bool
format_node_status(t_node_info *node_info, UXSQLExpBufferData *node_status, UXSQLExpBufferData *upstream, ItemList *warnings)
{
	bool error_found = false;
	t_node_info remote_node_rec = T_NODE_INFO_INITIALIZER;
	RecordStatus remote_node_rec_found = RECORD_NOT_FOUND;

	if (UXSQLstatus(node_info->conn) == CONNECTION_OK)
	{
	...
	switch (node_info->type)
	{
		case PRIMARY:
		{
				...
				if (node_info->active == true)
				{
					switch (node_info->recovery_type)
					{
						case RECTYPE_PRIMARY:
							appendUXSQLExpBufferStr(node_status, "* running");
							break;
						case RECTYPE_STANDBY:
							appendUXSQLExpBufferStr(node_status, "! running as standby");
							item_list_append_format(warnings,
													"node \"%s\" (ID: %i) is registered as primary but running as standby",
													node_info->node_name, node_info->node_id);
							break;
						case RECTYPE_UNKNOWN:
							appendUXSQLExpBufferStr(node_status, "! unknown");
							item_list_append_format(warnings,
			...
			else
			{
				...
				if (node_info->active == true)
				{
					appendUXSQLExpBufferStr(node_status, "? unreachable");
					item_list_append_format(warnings,
				...
				{
					appendUXSQLExpBufferStr(node_status, "- failed");
					error_found = true;
				}
			}
		}
				...
				if (node_info->active == true)
				{
					switch (node_info->recovery_type)
					{
						case RECTYPE_STANDBY:
							...
							break;
						case RECTYPE_PRIMARY:
							appendUXSQLExpBufferStr(node_status, "! running as primary");
							item_list_append_format(warnings,
		...
		case WITNESS:
		{
			...
			else if (node_info->node_status == NODE_STATUS_REJECTED)
			{
				if (node_info->active == true)
				{
					appendUXSQLExpBufferStr(node_status, "? rejected");
				}
				else
				{
					appendUXSQLExpBufferStr(node_status, "! failed");
		...
		case UNKNOWN:
		{
			...
			appendUXSQLExpBufferStr(node_status, "? unknown node type");
			error_found = true;
		}
		break;
	}
	...
	else if (remote_node_rec.type == WITNESS)
	{
		...
		{
			appendUXSQLExpBufferStr(upstream, "! ");
			item_list_append_format(warnings,
		...
		else
		{
			...
			{
				UXconn *upstream_conn = establish_db_connection_quiet(upstream_node_rec.conninfo);
	...
	else if (remote_node_rec.type == STANDBY)
	{
		...
		else
		{
			...
			{
				appendUXSQLExpBufferChar(upstream, '!');
				item_list_append_format(warnings,
...
{
	printf(_("%s: replication management tool for UXsinoDB\n"), progname());
	puts("");
...
do_help(void)
{
	...
	printf(_("  -b, --ux_bindir=PATH                path to UXsinoDB binaries (optional)\n"));
	...
	printf(_("  -D, --uxdata=DIR                    location of the node's data directory \n"));
...
bool
create_repmgr_extension(UXconn *conn)
{
	...
	UXconn	   *schema_create_conn = NULL;
	...
	res = UXSQLexec(schema_create_conn, "CREATE EXTENSION repmgr");

	if ((UXSQLresultStatus(res) != UXRES_COMMAND_OK && UXSQLresultStatus(res) != UXRES_TUPLES_OK))
	{
		log_error(_("unable to create \"repmgr\" extension:\n  %s"),
				  UXSQLerrorMessage(schema_create_conn));
	...
	if (is_superuser == false)
	{
		...
		res = UXSQLexec(schema_create_conn, query.data);
 ...
 */
int
check_server_version(UXconn *conn, char *server_type, bool exit_on_error, char *server_version_string)
{
	...
	if (conn_server_version_num < MIN_SUPPORTED_VERSION_NUM)
	{
		...
		{
			log_error(_("%s requires %s to be UxsinoDB %s or later"),
					  progname(),
					  server_type,
	 ...
	 * UxsinoDB from a particular UxsinoDB release onwards (e.g. 4.4 with UxsinoDB
	 * 12 and later due to recovery.conf removal), set MAX_UNSUPPORTED_VERSION and
	 * MAX_UNSUPPORTED_VERSION_NUM in "repmgr.h" to define the first UxsinoDB
	...
	if (conn_server_version_num >= MAX_UNSUPPORTED_VERSION_NUM)
	{
		if (conn_server_version_num > 0)
		{
			log_error(_("%s %s does not support UxsinoDB %s or later"),
					  progname(),
					  REPMGR_VERSION,
...
test_ssh_connection(char *host, char *remote_user)
{
	...
	for (i = 0; bin_true_paths[i] && r != 0; ++i)
	{
		...
		if (strlen(config_file_options.uxdb_password))
		{
			if (pipe(pfd) < 0)  
				return -1;   /* errno set by pipe() */ 
			r = runsshpass(config_file_options.uxdb_password, script, command, pfd);
		}
		else
			r = ux_system(script);
	}
 ...
 */
void
get_superuser_connection(UXconn **conn, UXconn **superuser_conn, UXconn **privileged_conn)
{
	t_connection_user userinfo = T_CONNECTION_USER_INITIALIZER;
...
get_standby_clone_mode(void)
{
	...
	if (*config_file_options.barman_host != '\0' && runtime_options.without_barman == false)
		...
		if (*config_file_options.ux_backupapi_host != '\0') {
			log_info("Attempting to use `ux_backupapi` new restore mode");
			mode = ux_backupapi;
		}
		else
			mode = ux_basebackup;
...
void
make_ux_path(UXSQLExpBufferData *buf, const char *file)
{
	appendUXSQLExpBuffer(buf, "%s%s",
					  ux_bindir, file);
}
...
{
	UXSQLExpBufferData rsync_flags;
	char		script[MAXLEN] = "";
	char		host_string[MAXLEN] = "";
	int			r = 0;

	initUXSQLExpBuffer(&rsync_flags);

	if (*config_file_options.rsync_options == '\0')
	{
		appendUXSQLExpBufferStr(&rsync_flags,
	...
	/*
	 * When copying the main UXDATA directory, certain files and contents of
	 * certain directories need to be excluded.
	 *
	 * See function 'sendDir()' in 'src/backend/replication/basebackup.c' -
	 * we're basically simulating what ux_basebackup does, but with rsync
	 * rather than the BASEBACKUP replication protocol command.
	 *
	 * *However* currently we'll always copy the contents of the 'ux_replslot'
	...
	if (is_directory)
	{
		...
		appendUXSQLExpBufferStr(&rsync_flags,
							 " --exclude=uxmaster.pid --exclude=uxmaster.opts --exclude=global/ux_control");

		appendUXSQLExpBufferStr(&rsync_flags,
							 " --exclude=recovery.conf --exclude=recovery.done");

		/*
		 * Ideally we'd use UX_AUTOCONF_FILENAME from utils/guc.h, but
		 * that has too many dependencies for a mere client program.
		 */
		appendUXSQLExpBuffer(&rsync_flags, " --exclude=%s.tmp",
						  UX_AUTOCONF_FILENAME);

		/* Temporary files which we don't want, if they exist */
		appendUXSQLExpBuffer(&rsync_flags, " --exclude=%s*",
						  UX_TEMP_FILE_PREFIX);
		...
		{
			appendUXSQLExpBufferStr(&rsync_flags,
								 " --exclude=ux_wal/* --exclude=log/*");
		}
		else
		{
			appendUXSQLExpBufferStr(&rsync_flags,
								 " --exclude=ux_xlog/* --exclude=ux_log/*");
		}

		/*
		 * From UxsinoDB 15, the core server no longer uses ux_stat_tmp,
		 * but some extensions (e.g. ux_stat_statements) may still do, so
		 * keep excluding it.
		 */
		appendUXSQLExpBufferStr(&rsync_flags,
							 " --exclude=ux_stat_tmp/*");
		...
		/*
		* #129060 建议repmgr clone实例时排除数据库日志目录log，避免日志混乱
		*
		* pg版本升级 ux_log -> log
		*/
		appendUXSQLExpBuffer(&rsync_flags, "%s",
						  " --exclude=log/* --exclude=ux_stat_tmp/*");
	...
	termUXSQLExpBuffer(&rsync_flags);
...
void
make_remote_repmgr_path(UXSQLExpBufferData *output_buf, t_node_info *remote_node_record)
{
	if (config_file_options.repmgr_bindir[0] != '\0')
	{
		int			len = strlen(config_file_options.repmgr_bindir);

		appendUXSQLExpBufferStr(output_buf,
							 config_file_options.repmgr_bindir);

		/* Add trailing slash */
		if (config_file_options.repmgr_bindir[len - 1] != '/')
		{
			appendUXSQLExpBufferChar(output_buf, '/');
		}
	}
	else if (ux_bindir[0] != '\0')
	{
		appendUXSQLExpBufferStr(output_buf,
							 ux_bindir);
	}

	appendUXSQLExpBuffer(output_buf,
					  "%s -f %s ",
					  progname(),
...
void
make_repmgrd_path(UXSQLExpBufferData *output_buf)
{
	if (config_file_options.repmgr_bindir[0] != '\0')
	{
		int			len = strlen(config_file_options.repmgr_bindir);
		...
		case ACTION_START:
			{
				...
				else
				{
					...
					make_ux_path(&command, "ux_ctl");

					appendUXSQLExpBuffer(&command,
									  " %s -w -D ",
									  config_file_options.ux_ctl_options);
 ...
 *
 * This is primarily intended for use with "ux_ctl" (which itself shouldn't
...
get_node_config_directory(char *config_dir_buf)
{
	...
	{
		strncpy(config_dir_buf, config_file_options.config_directory, MAXUXPATH);
		return;
	}

	if (config_file_options.data_directory[0] != '\0')
	{
		strncpy(config_dir_buf, config_file_options.data_directory, MAXUXPATH);
		return;
	}
...
get_node_data_directory(char *data_dir_buf)
{
	...
	if (config_file_options.data_directory[0] != '\0')
	{
		strncpy(data_dir_buf, config_file_options.data_directory, MAXUXPATH);
		return;
	}

	if (runtime_options.data_dir[0] != '\0')
	{
		strncpy(data_dir_buf, runtime_options.data_dir, MAXUXPATH);
		return;
	}
...
init_node_record(t_node_info *node_record)
{
	...
	strncpy(node_record->config_file, config_file_path, MAXUXPATH);

	/* uxdb: init the virtual ip */
	strncpy(node_record->virtual_ip, config_file_options.virtual_ip, MAXUXPATH);
	strncpy(node_record->network_card, config_file_options.network_card, MAXUXPATH);
	/* add by songjinzhou for #178952 at 2023/03/16 reveiwer houjiaxing. */
	strncpy(node_record->uxdb_passwd, config_file_options.uxdb_password, MAXUXPATH);
	strncpy(node_record->root_passwd, config_file_options.root_password, MAXUXPATH);
...
bool
can_use_ux_rewind(UXconn *conn, const char *data_directory, UXSQLExpBufferData *reason)
{
	bool		can_use = true;
	...
	{
		appendUXSQLExpBufferStr(reason,
	 ...
	 * checking the local ux_control file here as the value will be the same
 ...
 */
bool
create_replication_slot(UXconn *conn, char *slot_name, t_node_info *upstream_node_record, UXSQLExpBufferData *error_msg)
{
	UXconn *slot_conn = NULL;
	...
	if (runtime_options.dry_run == true)
	{
		switch (ReplicationSlotUser)
		{
			...
			{
				UXconn *repl_conn = duplicate_connection(conn,
														 replication_user,
														 true);
				if (repl_conn == NULL || UXSQLstatus(repl_conn) != CONNECTION_OK)
				{
					log_error(_("unable to create replication connection as user \"%s\""),
							  replication_user);
					log_detail("%s", UXSQLerrorMessage(repl_conn));
			...
			case SUPERUSER:
			{
				UXconn *superuser_conn = duplicate_connection(conn,
															  runtime_options.superuser,
															  false);
				if (superuser_conn == NULL || UXSQLstatus(superuser_conn )!= CONNECTION_OK)
				{
					log_error(_("unable to create superuser connection as user \"%s\""),
							  runtime_options.superuser);
					log_detail("%s", UXSQLerrorMessage(superuser_conn));
...
bool
drop_replication_slot_if_exists(UXconn *conn, int node_id, char *slot_name)
{
	...
	else
	{
		...
		bool use_replication_protocol = false;
		UXconn *slot_conn = _get_replication_slot_connection(conn,
															 replication_user,
...
static void
_determine_replication_slot_user(UXconn *conn, t_node_info *upstream_node_record, char **replication_user)
{
	/*
	...
	if (ReplicationSlotUser == USER_TYPE_UNKNOWN)
	{
		 ...
		 */
			 ...
			 && strncmp(upstream_node_record->repluser, UXSQLuser(conn), NAMEDATALEN) != 0)
		{
		...
		else
		{
			...
			{
				log_debug("%i %s %s", upstream_node_record->node_id, upstream_node_record->repluser, UXSQLuser(conn));
			}
			else
			{
...
static UXconn *
_get_replication_slot_connection(UXconn *conn, char *replication_user, bool *use_replication_protocol)
{
	UXconn *slot_conn = NULL;
	/*
...
bool
check_replication_slots_available(int node_id, UXconn* conn)
{
 ...
 */
standy_join_status
check_standby_join(UXconn *upstream_conn, t_node_info *upstream_node_record, t_node_info *standby_node_record)
{
	int i;
	...
	for (; i < config_file_options.node_rejoin_timeout; i++)
	{
		...
		if (i % 5 == 0)
		{
			...
			{
				log_detail(_("checking for record in node \"%s\"'s \"ux_stat_replication\" table where \"application_name\" is \"%s\""),
						   upstream_node_record->node_name,
						   standby_node_record->node_name);
			}
		}
 ...
 */
bool
check_node_can_attach(TimeLineID local_tli, XLogRecPtr local_xlogpos, UXconn *follow_target_conn, t_node_info *follow_target_node_record, bool is_rejoin)
{
	...
	if (local_system_identifier == UNKNOWN_SYSTEM_IDENTIFIER)
	{
		...
		if (runtime_options.dry_run == true)
		{
			log_warning(_("unable to retrieve system identifier from ux_control"));
		}
		else
		{
			log_error(_("unable to retrieve system identifier from ux_control, aborting"));
		}
	}
	 ...
	 * requires UxsinoDB 9.6 and later.
	 */
	if (follow_target_identification.timeline < local_tli)
	{
		...
		if (is_rejoin == false)
		{
			...
			if (UXSQLserverVersion(follow_target_conn) >= 90600)
			{
		...
		/*
		 * ux_rewind can only rejoin to a lower timeline from UxsinoDB 9.6
		 */
		if (UXSQLserverVersion(follow_target_conn) < 90600)
		{
			log_error(_("this node's timeline is ahead of the %s target node's timeline"), action);
			...
			if (runtime_options.force_rewind_used == true)
			{
				log_hint(_("ux_rewind can only be used to rejoin to a node with a lower timeline from UxsinoDB 9.6"));
			}
		...
		{
			log_notice(_("ux_rewind execution required for this node to attach to rejoin target node %i"),
	...
	else if (follow_target_identification.timeline == local_tli)
	{
		...
		else
		{
			 ...
			 * same timeline. Also, ux_rewind does not detect this situation,
			 * as there is no definitive fork point.
			 *
			 * Note that Uxdb will still happily attach to the upstream in state "streaming"
	...
	else
	{
		...
		if (local_xlogpos > follow_target_history->end)
		{
			...
			{
				log_hint(_("use --force-rewind to execute ux_rewind"));
			}
		}
 ...
 */
extern bool
check_replication_config_owner(int ux_version, const char *data_directory, UXSQLExpBufferData *error_msg, UXSQLExpBufferData *detail_msg)
{
	UXSQLExpBufferData replication_config_file;
	struct stat     dirstat;
	...
	if (stat(data_directory, &dirstat))
	{
		if (error_msg != NULL)
		{
			appendUXSQLExpBuffer(error_msg,
							  ...
							  data_directory);
			appendUXSQLExpBufferStr(detail_msg,
								 strerror(errno));
		}
		return false;
	}

	initUXSQLExpBuffer(&replication_config_file);

	appendUXSQLExpBuffer(&replication_config_file,
					  "%s/%s",
					  config_file_options.data_directory,
					  ux_version >= 120000 ? UX_AUTOCONF_FILENAME : RECOVERY_COMMAND_FILE);

	stat(replication_config_file.data, &confstat);

	if (confstat.st_uid == dirstat.st_uid)
	{
		termUXSQLExpBuffer(&replication_config_file);
		return true;
	}
	...
	if (error_msg != NULL)
	{
		...
		appendUXSQLExpBuffer(detail_msg,
 ...
 */
void
check_shared_library(UXconn *conn)
{
	...
	log_hint(_("ensure \"shared_preload_libraries\" includes \"repmgr\" and restart UXsinoDB"));
	UXSQLfinish(conn);
	exit(ERR_BAD_CONFIG);
}
...
bool
is_repmgrd_running(UXconn *conn)
{
	pid_t		pid;
...
parse_repmgr_version(const char *version_string)
{
	...
	UXSQLExpBufferData sscanf_string;

	initUXSQLExpBuffer(&sscanf_string);

	appendUXSQLExpBuffer(&sscanf_string, "%s ",
					  progname());
	appendUXSQLExpBufferStr(&sscanf_string, "%i.%i.%i");
	...
	{
		resetUXSQLExpBuffer(&sscanf_string);
		appendUXSQLExpBuffer(&sscanf_string, "%s ",
						  progname());
		appendUXSQLExpBufferStr(&sscanf_string, "%i.%i");
	...
	termUXSQLExpBuffer(&sscanf_string);
