...
/* #140905 repmgr编译时生成的config.h中的相关宏在实际代码中并未使用,包括:
 *    #define PACKAGE_BUGREPORT "repmgr@googlegroups.com"
 *    #define PACKAGE_NAME "repmgr"
 *    #define PACKAGE_STRING "repmgr 4.3"
 *    #define PACKAGE_TARNAME "repmgr"
 *    #define PACKAGE_URL "https://repmgr.org/"
 *    #define PACKAGE_VERSION "4.3"
 因此通过直接屏蔽的方式,避免和内核config.h和ux_config.h的重定义警告
*/
#if 0
...
#define DEFAULT_STANDBY_FOLLOW_TIMEOUT       60  /* seconds */
#define DEFAULT_STANDBY_FOLLOW_RESTART       false
#define DEFAULT_STANDBY_UX_BACKUPAPI_OP_TYPE "recovery"
#define DEFAULT_STANDBY_FOLLOW_RESTART       false
...
#define DEVICE_CHECK_TIMEOUT                 60  /* seconds */  /* uxdb */
#define DEVICE_CHECK_TIMES                   3   /* times */    /* uxdb */
#define DEFAULT_STANDBY_WAIT_TIMEOUT         10  /* mins */ /* uxdb */
...
/* 封装system函数, 解决某些环境下可能由于SIGCHLD信号导致的返回结果异常问题 */
typedef void (*sighandler_t)(int);

static inline int
ux_system(const char *cmd)
{
	int ret = 0;
	sighandler_t old_handler;

	/* 调用system函数前将SIGCHLD信号设置为缺省值SIG_DFL并保存原信号值 */
	old_handler = signal(SIGCHLD, SIG_DFL);
	ret = system(cmd);
	/* system函数执行后恢复SIGCHLD信号为原来设置的值 */
