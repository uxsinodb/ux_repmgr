...
/* legacy options */
	...
	while ((c = getopt_long(argc, argv, "?Vf:L:vdp:sm", long_options, &optindex)) != -1)
	{
		switch (c)
		{
			...
			case 'p':
				strncpy(pid_file, optarg, MAXUXPATH);
				break;
			...
			case 'L':
				{
					...
					else
					{
						...
						appendUXSQLExpBuffer(&invalid_log_level,
	...
	if (no_pid_file == false)
	{
		if (config_file_options.repmgrd_pid_file[0] != '\0')
		{
			...
			{
				strncpy(pid_file, config_file_options.repmgrd_pid_file, MAXUXPATH);
			}
		}
		...
		if (pid_file[0] == '\0')
		{
			...
			char		package_pid_file[MAXUXPATH] = "";
	 ...
	 * sanity checks
	 *
	 * Note: previous repmgr versions checked the UXsinoDB version at this
	...
	/* warn about any settings which might not be relevant for the current UXsinoDB version  */
	if (config_file_options.standby_disconnect_on_failover == true && UXSQLserverVersion(local_conn) < 90500)
	{
		...
		log_detail(_("available from UXsinoDB 9.5, this UXsinoDB version is %i"), UXSQLserverVersion(local_conn));
	}
	...
	{
		/*
		 * extension is the latest available according to "ux_available_extensions" -
...
void
update_registration(UXconn *conn)
{
...
{
	uxsqlsignal(SIGHUP, handle_sighup);
	...
	{
		case REPLICATION_TYPE_PHYSICAL:
			uxsqlsignal(SIGINT, handle_sigint_physical);
			uxsqlsignal(SIGTERM, handle_sigint_physical);
			break;
	}
}
#endif
...
show_usage(void)
{
	fprintf(stderr, _("%s: replication management daemon for UXsinoDB\n"), progname());
...
show_help(void)
{
	printf(_("%s: replication management daemon for UXsinoDB\n"), progname());
	puts("");
...
bool
check_upstream_connection(UXconn **conn, const char *conninfo, UXconn **paired_conn)
{
	...
	if (config_file_options.connection_check_type != CHECK_QUERY)
	{
		...
		else if (config_file_options.connection_check_type == CHECK_CONNECTION)
		{
			 ...
			 */
			UXconn *test_conn = UXSQLconnectdb(conninfo);

			log_debug("check_upstream_connection(): attempting to connect to \"%s\"", conninfo);

			if (UXSQLstatus(test_conn) != CONNECTION_OK)
			{
				log_warning(_("unable to connect to \"%s\""), conninfo);
				log_detail("\n%s", UXSQLerrorMessage(test_conn));
				success = false;
			}
			UXSQLfinish(test_conn);
		}
	...
	for (;;)
	{
		...
		else
		{
			...
			if (UXSQLsendQuery(*conn, config_file_options.connection_check_query) == 0)
			{
...
void
try_reconnect(UXconn **conn, t_node_info *node_info)
{
	UXconn	   *our_conn;
	...
	for (i = 0; i < max_attempts; i++)
	{
		...
		if (is_server_available_params(&conninfo_params) == true)
		{
			...
			if (UXSQLstatus(our_conn) == CONNECTION_OK)
			{
				...
				/* BEGIN:  Added by songjinzhou for #132171、160873、169199, 2022/1/24  reviewer:zdl */
				/* 在短暂的断网之后 重启过程 会调用try_reconnect 这里需要绑定vip */
				/* Only primary node need to bind vip */
				/* standby node can reconnect primary node or upstream node,
				 * so the node id and node type determine whether the node is the primary node.
				 */
				if(( node_info->node_id == local_node_info.node_id && node_info->type == PRIMARY) &&
							check_vip_conf(config_file_options.virtual_ip, config_file_options.network_card))

				{
					bind_virtual_ip(config_file_options.virtual_ip, config_file_options.network_card);
				}
				/* END:  Added by songjinzhou for #132171, 2022/1/24  reviewer:zdl */
