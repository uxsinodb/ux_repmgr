...
static bool _local_command(const char *command, UXSQLExpBufferData *outputbuf, bool simple, int *return_value);


/*
 * Execute a command locally. "outputbuf" should either be an
 * initialised UXSQLExpPuffer, or NULL
 */
bool
local_command(const char *command, UXSQLExpBufferData *outputbuf)
{
	return _local_command(command, outputbuf, false, NULL);
}

bool
local_command_return_value(const char *command, UXSQLExpBufferData *outputbuf, int *return_value)
{
...
bool
local_command_simple(const char *command, UXSQLExpBufferData *outputbuf)
{
...
static bool
_local_command(const char *command, UXSQLExpBufferData *outputbuf, bool simple, int *return_value)
{
	FILE	   *fp = NULL;
	char		output[MAXLEN];
	int			retval = 0;
	bool		success;
	char		tmpfile_path[MAXUXPATH];
	const char *tmpdir = getenv("TMPDIR");
	int			fd;
	UXSQLExpBufferData command_final;
	...
	initUXSQLExpBuffer(&command_final);
	appendUXSQLExpBufferStr(&command_final, command);

	appendUXSQLExpBuffer(&command_final, " 2>%s", tmpfile_path);

	log_verbose(LOG_DEBUG, "executing:\n  %s", command_final.data);

	if (outputbuf == NULL)
	{
		retval = ux_system(command_final.data);
		termUXSQLExpBuffer(&command_final);
	...
	{
		appendUXSQLExpBufferStr(outputbuf, output);
	...
	success = runsshpass(passwd, command, command_shell, pfd);
...
bool
local_command_sshpass(const char *command, const char *command_shell, UXSQLExpBufferData *outputbuf, const char *passwd)
{
	return _local_command_sshpass(command, command_shell, outputbuf, passwd);
}
 ...
 */
bool
remote_command(const char *host, const char *user, const char *command, const char *ssh_options, UXSQLExpBufferData *outputbuf)
{
	FILE	   *fp;
    ...
    appendUXSQLExpBuffer(&ssh_command,
	...
	{
		int pfd[2];
		UXSQLExpBufferData ssh_host;

		initUXSQLExpBuffer(&ssh_host);

		if (*user != '\0')
		{
			appendUXSQLExpBuffer(&ssh_host, "%s@", user);
		}

		appendUXSQLExpBufferStr(&ssh_host, host);

		appendUXSQLExpBuffer(&ssh_command,
							 "ssh -o StrictHostKeyChecking=no %s uxdb@%s",
							 ssh_options,
							 ssh_host.data);
		termUXSQLExpBuffer(&ssh_host);
...
void
make_remote_command(const char *host, const char *user, const char *command, const char *ssh_options, UXSQLExpBufferData *ssh_command)
{
	UXSQLExpBufferData ssh_host;
...
pid_t
disable_wal_receiver(UXconn *conn)
{
	char buf[MAXLEN];
	...
	get_ux_setting(conn, "wal_retrieve_retry_interval", buf);
...
pid_t
enable_wal_receiver(UXconn *conn, bool wait_startup)
{
	char buf[MAXLEN];
	...
	if (get_ux_setting(conn, "wal_retrieve_retry_interval", buf) == false)
	{
