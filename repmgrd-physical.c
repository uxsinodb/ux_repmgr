...
static UXconn *upstream_conn = NULL;
static UXconn *primary_conn = NULL;
static bool touch_timeout_label = false; //uxdb
static instr_time unreachable_sync_standby_start; //uxdb
...
static UXconn *vip_conn = NULL;
static char vip_conninfo[MAXLEN] = {0};
static bool network_card_is_down = false;
...
static void check_connection(t_node_info *node_info, UXconn **conn);
...
static void handle_sighup(UXconn **conn, t_server_type server_type);
...
static void parse_failover_validation_command(const char *template,  t_node_info *node_info, election_stats *stats, UXSQLExpBufferData *out);
static bool check_node_can_follow(UXconn *local_conn, XLogRecPtr local_xlogpos, UXconn *follow_target_conn, t_node_info *follow_target_node_info);
...
static int try_primary_reconnect(UXconn **conn, UXconn *local_conn, t_node_info *node_info);

//uxdb
static void check_disk(NodeInfoList *node_list);
static void signalAlarm(int);
static bool check_network_card_status(UXconn *conn, int node_id);
static bool check_service_status_command(const char *command, UXSQLExpBufferData *outputbuf);
static NodeStatus check_service_status_is_shutdown_cleanly(const char *node_status_output, XLogRecPtr *checkPoint);
static void exec_node_rejoin_primary(NodeInfoList *my_node_list);
static void check_sync_async(NodeInfoList *my_node_list);
static BS_ACTION check_BS(NodeInfoList *my_node_list);
static TL_RET check_timeline(UXconn *remote_conn,t_node_info *peer_node_info);
static bool write_standby_signal(bool *current_standby_flag);
static void delete_standby_single_file(bool current_standby_flag);
static bool isexist_file(char * filename);
static void get_vip_conninfo(char *conninfo, t_node_info *node_info);
static bool check_vip_connection(char *conninfo, UXconn **conn);
static void retry_vip_connent(char *conninfo, UXconn **conn);
static bool is_primary_node_rejoin(char *conninfo, UXconn **conn);
static bool is_primary_node_update(UXconn *conn, int upstream_node_id);
static void stop_the_service(void);
static int get_match_async_node_num(NodeInfoList *my_node_list, char nodename[][MAXLEN]);
static int parser_nums_synchronous_standby_names(char *sync_names);
static int parser_info_synchronous_standby_names(char *sync_names);
static bool parser_nodename_synchronous_standby_names(char *sync_names, char nodename[][MAXLEN]);
...
{
	UXconn *writeable_conn;
	UXSQLExpBufferData event_details;

	initUXSQLExpBuffer(&event_details);

	appendUXSQLExpBuffer(&event_details,
					  _("%s signal received"),
					  uxdb_signal_arg == SIGTERM
					  ? "TERM" : "INT");
	...
	if (UXSQLstatus(writeable_conn) == CONNECTION_OK)
	...
	termUXSQLExpBuffer(&event_details);
...
void
do_physical_node_check(UXconn *conn)
{
	/*
	...
	if (local_node_info.active == false)
	{
		...
		{
			UXconn *primary_conn = get_primary_connection(conn, NULL, NULL);
			bool	success = true;
		...
		if (local_node_info.active == false)
		{
			switch (config_file_options.failover)
			{
				...
				case FAILOVER_AUTOMATIC:
					...
					//uxdb: comment out the terminate, monitoring logic may have chance to update inactive to active 
					//terminate(ERR_BAD_CONFIG);
					break;
	...
	if (config_file_options.failover == FAILOVER_AUTOMATIC)
	{
		...
		if (config_file_options.promote_command[0] == '\0')
		{
			...
			if (config_file_options.service_promote_command[0] != '\0')
			{
				 ...
				 * level promote command (e.g. ux_ctlcluster).
				 *
...
monitor_streaming_primary(void)
{
	...
	NodeInfoList mynodes = T_NODE_INFO_LIST_INITIALIZER;	//uxdb
...

	{
		...
		else
		{
			...
			for (cell = db_child_node_records.head; cell; cell = cell->next)
			{
				 ...
				 * in ux_stat_replication, we can't know whether it has become detached, or
	...
	/* uxdb: read nodes for auto rejoin */
	...
	while (true)
	{
		...
		/* BEGIN:  Added by huyn for #184528, 2023/5/18  reviewer:houjx */
		/*  user, db, connect_timeout, vip form vip conninfo */
		if (vip_conninfo[0] == '\0')
		{
			get_vip_conninfo(vip_conninfo, &local_node_info);
			log_debug(_("get vip_connection info is %s"), vip_conninfo);
		}
		/* END:  Added by huyn for #184528, 2023/5/18  reviewer:houjx */

		//uxdb check disk is writable
		if(is_server_available(local_node_info.conninfo) && (local_node_info.node_status == NODE_STATUS_UP))
		{
			ux_get_all_node_records(local_conn, &mynodes);
			check_disk(&mynodes);
		}

		/* is_primary_node_rejoin 当主备切换之后，原主节点可以通过VIP连上新的主节点，导致原主节点没有重新rejoin */
		/* uxdb: to prevent network is down while db is still accessible locally
		 * further check network state
		 */
		if (((UXSQLstatus(local_conn) != CONNECTION_OK) && is_primary_node_rejoin(vip_conninfo, &vip_conn)) || 
		   (check_network_card_status(local_conn, local_node_info.node_id)==false))
		{
			...
			{
				UXSQLExpBufferData command_str;
...

				{
					...
					appendUXSQLExpBufferStr(&event_details,
				...
				/* uxdb: for db issue, try to reconect 
				 * for network issue, go to degraded directly
				 * */
				/* Modify by huyn for #184528, 2023/5/18  reviewer:houjx */
				if(UXSQLstatus(local_conn) != CONNECTION_OK)
				{
					try_reconnect(&local_conn, &local_node_info);
					/* try to connect with vip */
					if (UXSQLstatus(local_conn) != CONNECTION_OK)
						retry_vip_connent(vip_conninfo, &vip_conn);
				}
				...
				if (local_node_info.node_status == NODE_STATUS_UP)
				{
					...
					if (check_primary_status(NO_DEGRADED_MONITORING_ELAPSED) == false)
					{
						...
						clear_node_info_list(&mynodes);
						clear_child_node_info_list(&local_child_nodes);
						return;
					}
				...
				/* uxdb: When old primary node has stopped from replication cluster, unbind the virtual ip */
				if(unbind_virtual_ip(config_file_options.virtual_ip, config_file_options.network_card, config_file_options.uxdb_password))
				...
				log_notice(_("unable to connect to local node, falling back to degraded monitoring"));
				 /* uxdb: When old primary node is in degraded mode, stop the service*/
				initUXSQLExpBuffer(&command_str);
				appendUXSQLExpBuffer(&command_str,
						"%s/ux_ctl -D %s stop -m fast",
						config_file_options.ux_bindir, config_file_options.data_directory);
				ux_system(command_str.data);
				log_debug("%s/ux_ctl -D %s stop -m fast",
						config_file_options.ux_bindir, config_file_options.data_directory);
				termUXSQLExpBuffer(&command_str);
			...
			{
				/* uxdb: local node status has been set 'NODE_STATUS_DOWN'
				 * try node rejoin */
				log_debug("exec node rejoin, NODE_STATUS_DOWN");
				exec_node_rejoin_primary(&mynodes);
			}
		}
		else /* uxdb: local node is reachable */
		{
...
loop:
		...
		/* uxdb: refresh the node_list in case any new node registered or unregistered */
		if(is_server_available(local_node_info.conninfo) && (local_node_info.node_status == NODE_STATUS_UP))
		{
			ux_get_all_node_records(local_conn, &mynodes);
			...
			if(config_file_options.check_brain_split)
			{
				...
				/* BEGIN add by zhaoqg for #111224, 2021/07/28, reviewer:houjx */
				/* if do nothing, close the connection created by the split-brain check */
				else if (ret == DO_NOTHING)
				{
					NodeInfoListCell *mycell = NULL;

					for (mycell = mynodes.head; mycell; mycell = mycell->next)
				   {
						if ((mycell->node_info->conn != NULL) || (UXSQLstatus(mycell->node_info->conn) == CONNECTION_OK))
						{
							UXSQLfinish(mycell->node_info->conn);
							mycell->node_info->conn = NULL;
							continue;
						}
					}
				}
				/* END add by zhaoqg for #111224, 2021/07/28, reviewer:houjx */
			}
		}
...
check_primary_status(int degraded_monitoring_elapsed)
{
	...
	if (UXSQLstatus(new_primary_conn) != CONNECTION_OK)
	{
		...
		else
		{
			...
			log_detail("\n%s", UXSQLerrorMessage(new_primary_conn));
		}
	...
	/* check other type exist vip,if standby node has vip,unbind vip */
	if (local_node_info.type != PRIMARY && check_vip_conf(config_file_options.virtual_ip, config_file_options.network_card))
	{
		if (unbind_virtual_ip(config_file_options.virtual_ip, config_file_options.network_card))
		{
			log_notice(_("node %i unbind virtual vip successed"), config_file_options.node_id);
		}
	}
...
execute_child_nodes_disconnect_command(NodeInfoList *db_child_node_records, t_child_node_info_list *local_child_nodes)
{
	...
	if (connected_count < min_required_connected_count)
	{
		...
		if (child_nodes_disconnect_command_executed == false)
		{
			...
			{
				char parsed_child_nodes_disconnect_command[MAXUXPATH];
...
monitor_streaming_standby(void)
{
	...
	if (local_node_info.upstream_node_id == UNKNOWN_NODE_ID)
	{
		...
		{
			/* modifyed by houjiaxing for #188634 at 2023/07/21 reveiwer huyuanni. */
			log_error(_("unable to determine an active primary for this cluster, terminating sleep(5)"));
			//terminate(ERR_BAD_CONFIG);
			sleep(5);
		}
		...
		if (UXSQLstatus(upstream_conn) != CONNECTION_OK)
		{
			/* modifyed by houjiaxing for #188634 at 2023/07/21 reveiwer huyuanni. */
			log_error(_("unable connect to upstream node (ID: %i), terminating sleep(5)"),
					local_node_info.upstream_node_id);
	...
	else
	{
		...
		{
			/* modifyed by houjiaxing for #190759 at 2023/11/06 reveiwer wangbocai. */
			log_error(_("unable to retrieve record for upstream node (ID: %i), terminating sleep(5)"),
					  local_node_info.upstream_node_id);
			sleep(5);
		}
	...
	if (local_node_info.active == false)
	{
		...
		{
			/* Modified by chen_jingwen for #207866 at 2024/10/8 */
			/* 此次对于定义的变量并没有使用，由于不知道这么做的意义，所以暂时先注释代码 */
			// UXSQLExpBufferData event_details;
...
NO_UPSTREAM_LABEL:
	while (true)
	{
		...
		/* uxdb: check local and auto rejoin */
		if (is_server_available(local_node_info.conninfo) == false)
		{
			UXSQLExpBufferData node_rejoin_command_str;
			UXSQLExpBufferData output_buf;
			UXSQLExpBufferData check_clean_shutdown_str;
			bool    success = false;
			int     r;

			/* BEGIN Modify by liuxw for #204009 2024/03/14 reviewer:hanlx */
			initUXSQLExpBuffer(&output_buf);
			initUXSQLExpBuffer(&check_clean_shutdown_str);
			appendUXSQLExpBuffer(&check_clean_shutdown_str,
					"%s/repmgr node status -f %s --is-shutdown-cleanly;",
					config_file_options.repmgr_bindir, config_file);
			/* END Modify by liuxw for #204009 2024/03/14 reviewer:hanlx */

			success = check_service_status_command(check_clean_shutdown_str.data, &output_buf);
			termUXSQLExpBuffer(&check_clean_shutdown_str);
			...
			if (success == true)
			{
				...
				if (status == NODE_STATUS_UNCLEAN_SHUTDOWN)
				{
					...
					UXSQLExpBufferData stop_service_command_str;
					log_notice("unclean shutdown detected, start and stop db to clean");

					initUXSQLExpBuffer(&stop_service_command_str);
					/* Modified by zhangxin for #180836 on 2023/4/14, reviewer duankun */
					/* start和stop之间增加2秒延时，避免stop时collector进程未完全启动导致数据库异常退出 */
					appendUXSQLExpBuffer(&stop_service_command_str,
							"%s/ux_ctl -D %s start;sleep 2;",
							config_file_options.ux_bindir, config_file_options.data_directory);
					appendUXSQLExpBuffer(&stop_service_command_str,
							"%s/ux_ctl -D %s stop",
							config_file_options.ux_bindir, config_file_options.data_directory);
					/* Begin modified by Liuqq 2020/12/11 for #95577 */
					if (check_network_card_status(local_conn, local_node_info.node_id) != false)
					{
						 ux_system(stop_service_command_str.data);

						 termUXSQLExpBuffer(&stop_service_command_str);
					}
					/* End modified by Liuqq 2020/12/11 for #95577 */
				}
			}
			termUXSQLExpBuffer(&output_buf);

			initUXSQLExpBuffer(&node_rejoin_command_str);
			/* BEGIN Modify by liuxw for #204009 2024/03/14 reviewer:hanlx */
			/* begin:Modify by houjiaxing for #191581, 2023/11/23  reviewer:wangbocai */
			appendUXSQLExpBuffer(&node_rejoin_command_str,
					"%s/repmgr -d \'%s\' node rejoin -f %s --force-rewind",
					config_file_options.repmgr_bindir, upstream_node_info.conninfo, config_file);
			/* end:Modify by houjiaxing for #191581, 2023/11/23  reviewer:wangbocai */
			/* END Modify by liuxw for #204009 2024/03/14 reviewer:hanlx */

			if (local_node_info.failed_connect_times >= config_file_options.reconnect_attempts)
			{
				r = ux_system(node_rejoin_command_str.data);
				if (r != 0)
				{
			...
			termUXSQLExpBuffer(&node_rejoin_command_str);
		}
		else
		{
		...
		/* add by houjiaxing for #188733 at 2023/10/16 reveiwer wangbocai */
		...
		if (UXSQLstatus(local_conn) != CONNECTION_OK)
		{
			...
			if (UXSQLstatus(local_conn) != CONNECTION_OK)
			{
				...
				log_detail("\n%s", UXSQLerrorMessage(local_conn));
			}
		}
		...
		/* modify by houjiaxing for #188733 at 2023/10/16 reveiwer wangbocai */
		if (upstream_check_result == true && UXSQLstatus(local_conn) == CONNECTION_OK)
		{
			t_node_info confusion_node_info = T_NODE_INFO_INITIALIZER;
			...
			if (upstream_node_info.type == STANDBY
				&& local_node_info.upstream_node_id != upstream_node_info.upstream_node_id)
			{
				record_status = get_node_record(upstream_conn, upstream_node_info.upstream_node_id, &confusion_node_info);
				if (record_status != RECORD_FOUND)
					log_error(_("unable to retrieve metadata record found for node %i"), local_node_info.node_id);
				
				if (confusion_node_info.active == true && confusion_node_info.type == PRIMARY
				&& upstream_node_info.upstream_node_id == confusion_node_info.node_id)
					stop_the_service();
			}
			else if (upstream_node_info.type == PRIMARY &&
					 local_node_info.upstream_node_id != upstream_node_info.node_id)
			{
				record_status = get_node_record(upstream_conn, local_node_info.upstream_node_id, &confusion_node_info);
				if (record_status != RECORD_FOUND)
					log_error(_("unable to retrieve metadata record found for node %i"), local_node_info.node_id);

				if (confusion_node_info.active == true && confusion_node_info.type == STANDBY
					&& confusion_node_info.upstream_node_id == upstream_node_info.node_id)
					stop_the_service();
			}

			/* add by houjiaxing for #187183 at 2023/03/16 reveiwer wangbocai. */
			/* modify by wang_bocai for #206859 at 2024/5/22 */
			/* 确认主节点不是暂停状态，即并非是整个服务暂停（使用vip连接导致的暂停），则进行恢复 */
			if (repmgrd_is_paused(local_conn) && ! repmgrd_is_paused(upstream_conn))
				(void) repmgrd_pause(local_conn, false);
		}
		else
		{
			...
			if (upstream_node_info.node_status == NODE_STATUS_UP)
			{
				...
				if (upstream_node_info.node_status == NODE_STATUS_UP)
				{
					...
					if (upstream_node_info.type == PRIMARY)
					{
						...
						if (get_recovery_type(primary_conn) == RECTYPE_STANDBY)
						{
							...
							if (ping_result != UXRES_TUPLES_OK)
							{
								int i;
				...
				/* BEGIN:  Added by huyn for #184528, 2023/5/18  reviewer:houjx */
				/* standby cannot connent primary node ,but vip can connect primary
				 * node. maybe primary node network is invalid.but vip bind other network.
				 * so,standby node cannot autofailover
				 */
				if (vip_conninfo[0] != '\0')
				{
					UXconn *conn;
					conn = establish_db_connection(vip_conninfo, false);
					/* Modify by huyn for #185853, 2023/6/1  reviewer:houjx */
					if (UXSQLstatus(conn) == CONNECTION_OK && !is_primary_node_update(conn, upstream_node_info.node_id))
					{
						log_debug("pausing repmgrd on node %s (ID %i)",
								local_node_info.node_name,
								local_node_info.node_id);

						(void) repmgrd_pause(local_conn, true);						
					}
				...
				if (upstream_node_info.node_status == NODE_STATUS_DOWN)
				{
					...
					if (UXSQLstatus(local_conn) == CONNECTION_OK && repmgrd_is_paused(local_conn))
					{
			...
			/* Add by houjiaxing for #194511 at 2023/10/19 , reviewer:wangbocai */
			else if (upstream_node_info.node_status == NODE_STATUS_DOWN && network_card_is_down
			&& check_network_card_status(local_conn, local_node_info.node_id) == true)
			{
				bool failover_done = false;
				network_card_is_down = false;
				failover_done = do_primary_failover();
		...
		if (monitoring_state == MS_DEGRADED)
		{
			...
			if (upstream_check_result == true)
			{
				...
				if (UXSQLstatus(upstream_conn) == CONNECTION_OK)
				{
					...
					{
						if (primary_conn != NULL && UXSQLstatus(primary_conn) != CONNECTION_OK)
						{
							close_connection(&primary_conn);
						}
...
loop:
		...
		if (config_file_options.log_status_interval > 0)
		{
			...
			if (log_status_interval_elapsed >= config_file_options.log_status_interval)
			{
				UXSQLExpBufferData monitoring_summary;

				initUXSQLExpBuffer(&monitoring_summary);

				appendUXSQLExpBuffer(&monitoring_summary,
				...
				{
					appendUXSQLExpBufferStr(&monitoring_summary,
										 _(" (automatic failover disabled)"));
				}

				log_info("%s", monitoring_summary.data);
				termUXSQLExpBuffer(&monitoring_summary);
				...
				if (UXSQLstatus(primary_conn) == CONNECTION_OK && config_file_options.monitoring_history == true)
				{
		...
		if (UXSQLstatus(primary_conn) == CONNECTION_OK && config_file_options.monitoring_history == true)
		{
			...
			if (success == false && UXSQLstatus(primary_conn) != CONNECTION_OK && upstream_node_info.type == STANDBY)
			{
...
monitor_streaming_witness(void)
{
	...
	else
	{
		...
		log_detail("\n%s", UXSQLerrorMessage(primary_conn));
...
loop:
		...
		if(UXSQLstatus(local_conn) == CONNECTION_OK)	//uxdb
			(void) connection_ping(local_conn);
		...
		if (config_file_options.log_status_interval > 0)
		{
			...
			if (log_status_interval_elapsed >= config_file_options.log_status_interval)
			{
				...
				appendUXSQLExpBuffer(&monitoring_summary,
								  _("witness node \"%s\" (ID: %i) monitoring primary node \"%s\" (node ID: %i) in %s state"),
								  local_node_info.node_name,
								  local_node_info.node_id,
								  upstream_node_info.node_name,
								  upstream_node_info.node_id,
...
do_primary_failover(void)
{
	...
	if (check_network_card_status(local_conn, local_node_info.node_id) == false)
	{
		network_card_is_down = true;
		log_warning(_("network card error, election cancelled"));
		return false;
	}
	...
	if (config_file_options.standby_disconnect_on_failover == true)
	{
		 ...
		 * on UxsinoDB version and user permissions.
		 */
	...
	else if (election_result == ELECTION_WON)
	{
		...
		/* uxdb: When new primary node has promoted successful, bind virtual ip to the node's network card */
		if(failover_state==FAILOVER_STATE_PROMOTED)
		{
			...
			{
				if(bind_virtual_ip(config_file_options.virtual_ip, config_file_options.network_card, config_file_options.uxdb_password))
	...
	switch (failover_state)
	{
		case FAILOVER_STATE_PROMOTED:
			...
			/* 提升为主节点后，执行一次checkpoint，更新control文件的时间线 */
			/* BEGIN:  Added by huyn for #177313, 2023/2/17  reviewer:wangyh,zhangwj */
			new_primary_execute_checkpoint(local_conn);
...
do_upstream_standby_failover(void)
{
	...
	char		parsed_follow_command[MAXUXPATH] = "";
	...
	standby_follow_result = ux_system(parsed_follow_command);
...
promote_self(void)
{
	...
	{
		/* Modify by houjiaxing for #194511 at 2023/10/19 , reviewer:wangbocai */
		if (local_node_info.upstream_node_id == UNKNOWN_NODE_ID)
			record_status = get_node_record(local_conn, upstream_node_info.node_id, &failed_primary);
		else
	...
	for (cell = standby_nodes->head; cell; cell = cell->next)
	{
		...
		if (UXSQLstatus(cell->node_info->conn) != CONNECTION_OK)
		{
			log_warning(_("unable to reconnect to \"%s\" (ID: %i)"),
						cell->node_info->node_name,
						cell->node_info->node_id);
			log_detail("\n%s", UXSQLerrorMessage(cell->node_info->conn));
...
follow_new_primary(int new_primary_id)
{
	...
	if (UXSQLstatus(upstream_conn) == CONNECTION_OK)
	{
		...
		/* modify by houjiaxing for #190846 at 2023/03/16 reveiwer wangbocai. */
		/* 立即更新上流节点信息 */
		if (primary_recovery_type == RECTYPE_PRIMARY)
		{
			new_primary_ok = true;
			log_notice(_("attempting to update upstream_node_info with new primary \"%s\" (node ID: %i)"),
						  new_primary.node_name,
						  new_primary_id);
			...
			if (record_status != RECORD_FOUND)
			{
				log_error(_("unable to retrieve metadata record for new primary (ID: %i)"),
							 local_node_info.upstream_node_id);
				return FAILOVER_STATE_FOLLOW_FAIL;
			}
		}
	...
	r = ux_system(parsed_follow_command);

	if (r != 0)
	{
		UXconn	   *old_primary_conn;
		...
		if (UXSQLstatus(old_primary_conn) == CONNECTION_OK)
		{
	...
	if (local_conn == NULL || UXSQLstatus(local_conn) != CONNECTION_OK)
	{
		log_error(_("unable to reconnect to local node \"%s\" (ID: %i)"),
				  local_node_info.node_name,
				  local_node_info.node_id);
...
do_election(NodeInfoList *sibling_nodes, int *new_primary_id)
{
	...
	UXSQLExpBufferData nodes_with_primary_visible;
	...
	initUXSQLExpBuffer(&nodes_with_primary_visible);
	...
	for (cell = sibling_nodes->head; cell; cell = cell->next)
	{
		 ...
		 *
		 * NOTE: from Ux12 we could execute "ux_promote()" from a running repmgrd;
		...
		if (sibling_replication_info.in_recovery == false && cell->node_info->type != WITNESS)
		{
			...
			if (can_follow == true)
			{
				*new_primary_id = cell->node_info->node_id;
				termUXSQLExpBuffer(&nodes_with_primary_visible);
		...
		if (sibling_replication_info.upstream_last_seen >= 0 && sibling_replication_info.upstream_last_seen < (config_file_options.monitor_interval_secs * 2))
		{
			...
			else
			{
				...
				log_notice(_("%s node \"%s\" (ID: %i) last saw primary node %i second(s) ago, considering primary still visible"),
						   ...
						   cell->node_info->node_id,
						   sibling_replication_info.upstream_last_seen);
				appendUXSQLExpBuffer(&nodes_with_primary_visible,
...
static void
check_connection(t_node_info *node_info, UXconn **conn)
{
	...
	/* 存在部分情况连接改变，但是repmgr pid没有重设，这里对这种场景进行检查处理 */
	if (*conn != NULL)
	{
		int stored_local_node_id = UNKNOWN_NODE_ID;
...
static void
handle_sighup(UXconn **conn, t_server_type server_type)
{
...
{
	UXSQLExpBufferData failover_validation_command;
	UXSQLExpBufferData command_output;
	int return_value = -1;

	initUXSQLExpBuffer(&failover_validation_command);
	...
	termUXSQLExpBuffer(&failover_validation_command);
...
static void
parse_failover_validation_command(const char *template, t_node_info *node_info, election_stats *stats, UXSQLExpBufferData *out)
{
	const char *src_ptr;
		...
		if (*src_ptr == '%')
		{
			switch (src_ptr[1])
			{
				case '%':
					/* %%: replace with % */
					src_ptr++;
					appendUXSQLExpBufferChar(out, *src_ptr);
					break;
				case 'n':
					/* %n: node id */
					src_ptr++;
					appendUXSQLExpBuffer(out, "%i", node_info->node_id);
					break;
				case 'a':
					/* %a: node name */
					src_ptr++;
					appendUXSQLExpBufferStr(out, node_info->node_name);
					break;

				case 'v':
					/* %v: visible nodes count */
					src_ptr++;
					appendUXSQLExpBuffer(out, "%i", stats->visible_nodes);
					break;
				case 'u':
					/* %u: shared upstream nodes count */
					src_ptr++;
					appendUXSQLExpBuffer(out, "%i", stats->shared_upstream_nodes);
					break;
				case 't':
					/* %t: total nodes count */
					src_ptr++;
					appendUXSQLExpBuffer(out, "%i", stats->all_nodes);
					break;
 ...
 * functionality (including log output related to --dry-run, ux_rewind etc.)
 * which we don't want here.
 */
static bool
check_node_can_follow(UXconn *local_conn, XLogRecPtr local_xlogpos, UXconn *follow_target_conn, t_node_info *follow_target_node_info)
{
	UXconn	   *local_repl_conn = NULL;
	t_system_identification local_identification = T_SYSTEM_IDENTIFICATION_INITIALIZER;

	UXconn	   *follow_target_repl_conn = NULL;
	...
	if (UXSQLstatus(follow_target_repl_conn) != CONNECTION_OK)
	{
		log_error(_("unable to establish a replication connection to the follow target node"));

		UXSQLfinish(follow_target_repl_conn);
		return false;
	}
...
check_witness_attached(t_node_info *node_info, bool startup)
{
	 ...
	 */
	UXconn *witness_conn = establish_db_connection_quiet(node_info->conninfo);

	if (UXSQLstatus(witness_conn) == CONNECTION_OK)
	{
...
{
	t_child_node_info *child_node = ux_malloc0(sizeof(t_child_node_info));
...
int
try_primary_reconnect(UXconn **conn, UXconn *local_conn, t_node_info *node_info)
{
	...
	for (i = 0; i < max_attempts; i++)
	{
		...
		if (is_server_available_params(&conninfo_params) == true)
		{
			...
			if (UXSQLstatus(our_conn) == CONNECTION_OK)
			{
				...
				if (UXSQLstatus(*conn) == CONNECTION_BAD)
				{
 ...
 * Modify by houjiaxing for #190369 at 2023/8/8 , reviewer:wangbocai
 */
static
void check_disk(NodeInfoList *node_list)
{
	UXSQLExpBufferData disk_check_command_str;
	NodeInfoListCell *mycell = NULL;
	bool found_other_active_db = false;
	int	ret = -1;
	int 	i;

	/* device_check_times 小于等于零时，不进行检测 */
	if (config_file_options.device_check_times <= 0)
	{
		return;
	}

	initUXSQLExpBuffer(&disk_check_command_str);
	/* add by houjiaxing for #190462 at 2023/08/15 reveiwer wangbocai. */
	appendUXSQLExpBuffer(&disk_check_command_str,
			"rm -f %s/ux_repmgr_test;", config_file_options.data_directory);
	appendUXSQLExpBuffer(&disk_check_command_str,
			"touch %s/ux_repmgr_test", config_file_options.data_directory);
	...
	{
		touch_timeout_label = false;
		alarm(config_file_options.device_check_timeout);
		ret = ux_system(disk_check_command_str.data);
		alarm(0);
//        if (config_file_options.log_switch)
//            log_notice("%d times try, result is %d... ", i + 1, r);

		/* 执行失败或者超时，将继续尝试下一次执行 */
		if (ret != 0 || true == touch_timeout_label)
		{
			log_error(_("failed to exec ux_system(touch), ret: %d errno: %d"), ret, errno);
...
*/
	...
	{	
		...
		log_warning(_("UXDATA in which storage is not working, ret: %d"), ret);

		/* 如果执行超时，额外打印报错信息 */
		if (true == touch_timeout_label)
		{
			log_detail(_("cannot write in %s after %d seconds"),
				config_file_options.data_directory,
				config_file_options.device_check_timeout);

			touch_timeout_label = false;
		}

		/* 循环遍历整个节点链表，确保此时还有其他活跃节点 */
		for(mycell = node_list->head; mycell; mycell = mycell->next)
		{
			mycell->node_info->conn = establish_db_connection(mycell->node_info->conninfo, false);

			if ((mycell->node_info->node_id == local_node_info.node_id) ||
				(mycell->node_info->type == WITNESS) ||
				(NULL == mycell->node_info->conn) ||
				(CONNECTION_OK != UXSQLstatus(mycell->node_info->conn)))
			{
				UXSQLfinish(mycell->node_info->conn);
				mycell->node_info->conn = NULL;
			}
		...
		/* 只有当本节点不是最后一个活跃节点时，才执行kill操作停止数据库 */
		if (found_other_active_db)
		{
			/*stop locale primary service*/
			initUXSQLExpBuffer(&stop_service_command_str);
			/* ADD by huyn for #172310 at 2022.12.9, reviewer by songjz */
			appendUXSQLExpBuffer(&stop_service_command_str,
				"head -n 1 %s/uxmaster.pid |xargs kill -9", config_file_options.data_directory);

			rt = ux_system(stop_service_command_str.data);

			log_notice("kill -9 uxdb service result:%d\n", rt);

			if (rt ==0)
				log_warning(_("local UXDB Database server is stopped")); 
		...
		{
			log_warning(_("failed to write to data directory, but this last surviving node will still run"));
		}
	}
	return;
}
...
/*
 * uxdb: alarm called func
 */
static void 
signalAlarm(int signum)
{
	touch_timeout_label = true;
}
 ...
 */
static bool
check_network_card_status(UXconn *conn, int node_id)
{
	FILE *pf = NULL;
	char    network_card[MAXLEN] = "";
	UXSQLExpBufferData str;
	int value;
	...
	{
		initUXSQLExpBuffer(&str);
		appendUXSQLExpBuffer(&str, "/sys/class/net/%s/carrier", network_card);
			
		pf = fopen(str.data, "r");
				termUXSQLExpBuffer(&str);
		...
		fclose(pf);
			...
			if (value != 1)
			{
				log_warning(_("end check network card: %s,return false, value:%d, status is DOWN "), pNetworkcardsArray[i], value);
				card_status = false;
			}
		}
		else
...
/*
 * uxdb: check servce status command
 * tianbing
 */
static bool
check_service_status_command(const char *command, UXSQLExpBufferData *outputbuf)
{
	FILE       *fp = NULL;
	char        output[MAXLEN];
	int         retval = 0;
	...
	if (outputbuf == NULL)
	{
		retval = ux_system(command);
		if(retval == 0)
			return true;
		else
		{
			log_error(_("ux_system() unable to execute local command:\n%s, ret=%d"), command, retval);
			return false;
		}
	}
	...
	if (fp == NULL)
	{
		log_error(_("popen() unable to execute local command:\n%s"), command);
		return false;
	}

	while (fgets(output, MAXLEN, fp) != NULL)
	{
		appendUXSQLExpBuffer(outputbuf, "%s", output);
		if (!feof(fp))
		{
			break;
		}
...
/*
 * uxdb: function that auto exec 'node rejoin'
 * tianbing
 */
static void 
exec_node_rejoin_primary(NodeInfoList *my_node_list)
{
	...
	if (success == true)
	{
		...
		if (status == NODE_STATUS_UNCLEAN_SHUTDOWN)
		{
			/*start and stop the service*/
			UXSQLExpBufferData stop_service_command_str;
			bool standby_file_flag = false;
			log_notice("unclean shutdown detected, start and stop db to clean");

			/* BEGIN:  Added by huyn for #176436, 2023/2/10  reviewer:zhangwj,wangyh */
			if (write_standby_signal(&standby_file_flag) == false)
			{
				log_error(_("unable to write \"standby.signal\" file"));
			}
			/* END:  Added by huyn for #176436, 2023/2/10  reviewer:zhangwj,wangyh */
			...
			termUXSQLExpBuffer(&stop_service_command_str);
			/* BEGIN:  Added by huyn for #176436, 2023/2/10  reviewer:zhangwj,wangyh */
			/* 在执行完数据库的启动停止后，删除掉standby.single */
			delete_standby_single_file(standby_file_flag);
			/* END:  Added by huyn for #176436, 2023/2/10  reviewer:zhangwj,wangyh */
		}
	}
	termUXSQLExpBuffer(&output_buf);
	...
	for (mycell = my_node_list->head; mycell; mycell = mycell->next)
	{
		...
		appendUXSQLExpBuffer(&node_rejoin_command_str,
							"%s/repmgr -d \'%s\' node rejoin -f %s --force-rewind",
							config_file_options.repmgr_bindir, mycell->node_info->conninfo, config_file);
		/* end:Modify by houjiaxing for #191581, 2023/11/23  reviewer:wangbocai */

		log_debug("try repmgr -d \'%s\' node rejoin --force-rewind -f %s",mycell->node_info->conninfo, config_file);
		/* END Modify by liuxw for #204009 2024/03/14 reviewer:hanlx */
		r = ux_system(node_rejoin_command_str.data);
		if (r != 0)
		{
 ...
 *
 * #202234 调整同步异步模式切换规则
 *  (注：本次只支持以下常用规则的切换,uxdb中可能还支持其他形式的配置,这些我们不再这里支持,也不建议在生产环境使用更复杂的配置方式)
 * 切换规则:
 *  a、当 synchronous_standby_names 有值,且满足以下条件需要切换 sync -> async
 *    1)值为 'node1,node2,...' 或 '(node1,node2,...)':时, 当前存活的流复制(sync + potential)节点数 < 1 时
 *    2)值为 'first n (*)' 或 'first n (node1,node2,...)', 当前存活的流复制(sync + potential)节点数 < n 时
 *    3)值为 'any n (*)' 或 'any n (node1,node2,...)':, 当前存活的流复制(quorum)节点数 < n 时
 *    4)其他场景均不切换
 *
 *  b、当 synchronous_standby_names 无值, synchronous_standby_names_record 有值, 且满足以下条件时需要切换 async -> sync
 *    (代表集群之前存在同步节点,目前全是异步状态)
 *    1)值为 'node1,node2,...' 或 '(node1,node2,...)':
 *       若配置中的节点存活数 >=1 时,且其中 >=1 个节点的数据流延迟达到要求
 *    2)值为 'first n (*)':
 *       若目前存在的流复制节点数 >=n 时,且其中 >=n 个节点的数据流延迟达到要求
 *    3)值为 'first n (node1,node2,...)':
 *       若配置中的节点的存活数 >= n 时,且其中 >=n 个节点的数据流延迟达到要求
 *    4)值为 'any n (*)' : 
 *       若目前存在的流复制节点数 >=n 时, 且其中 >=n 个节点的数据流延迟达到要求
 *    5)值为 'any n (node1,node2,...)':
 *       若配置中的节点的存活数 >= n 时,且其中 >=n 个节点的数据流延迟达到要求
 *    6)其他场景均不切换
 *
 *  c、当 synchronous_standby_names 和 synchronous_standby_names_record 均无值(代表集群一直是异步状态)
 *    1)无需切换
 */
static void
check_sync_async(NodeInfoList *my_node_list)
{
	char sync_names[MAXLEN] = { 0 };
	char sync_names_record[MAXLEN] = { 0 };
	int nums = 0;  
	int nums_record = 0;  /* 同步配置的要求的最少节点数量 */
	UXresult *res = NULL;
	int unreachable_standby_elapsed;
	static short unreachable_standby_counts = 0;
	int max_attempts = config_file_options.try_synchronous_connection_timeout;
	bool success = false;
	int records = -1;
	int switch_mode = 0; /* 0: do nothing, 1: check sync->async, 2: check async->sync */
	int sync_potential_nums = 0;
	int quorum_nums = 0;
	int confinfo;
	char node_names[NODENUMS + 1][MAXLEN] = { {0} };  /* 当配置具体节点名称时,目前支持最多10个节点的管理,配置'*'时无限制 */
	static bool pflag = true;

	/* 参数获取失败,直接返回 */
	success = get_ux_setting(local_conn, "synchronous_standby_names", sync_names);
	if(!success)
	{
		log_error(_("can not obtain 'synchronous_standby_names' value, do not switch repliction mode"));
		return;
	}

	success = get_ux_setting(local_conn, "synchronous_standby_names_record", sync_names_record);
	if(!success)
	{
		/* 旧版本无此参数,对旧版本做部分兼容:支持同步切异步,不支持异步切同步 */
		log_warning(_("can not obtain 'synchronous_standby_names_record' value"));
		nums_record = -1;
	}
	else
	{
		/* synchronous_standby_names_record 代表高可用部署的初始状态,若无值,则没有同步节点配置,无需切换 */
		nums_record = parser_nums_synchronous_standby_names(sync_names_record);
		if(0 == nums_record)
		{
			/* 每次守护进程启动只打印一次此信息 */
			if(pflag)
			{
				pflag = false;
				log_info(_("no requirement of switching repliction mode for synchronous configuration"));
			}
			return;
		}
	}

	/* 根据当前参数值,判断后面需要做哪种切换检查 */
	nums = parser_nums_synchronous_standby_names(sync_names);
	if(nums != 0)
		switch_mode = 1;  /* 后续做同步切异步的检查 */
	else
		switch_mode = 2;  /* 后续做异步切同步的检查 */

	if(switch_mode ==2 && nums_record == -1)
	{
		log_warning(_("incompatible with older versions, unable to switch async to sync mode"));
		return;
	}
	...
	{
		case 1:
			res = UXSQLexec(local_conn, "SELECT count(*) FROM ux_stat_replication WHERE sync_state = 'sync' or sync_state = 'potential'" 
										" UNION ALL SELECT count(*) FROM ux_stat_replication WHERE sync_state = 'quorum';");
			if (UXSQLresultStatus(res) != UXRES_TUPLES_OK)
			{
				log_error(_("unable to execute query at switch_mode: %d"), switch_mode);
				/* Added by chen_jingwen for #207866 at 2024/10/8 */
				UXSQLclear(res);
				res = NULL;
				return;
			}
			sync_potential_nums = atoi(UXSQLgetvalue(res, 0, 0));
			quorum_nums = atoi(UXSQLgetvalue(res, 1, 0));
			/* sync_potential_nums 和 quorum_nums 必有一个 0,而 nums 必大于0*/
			if (quorum_nums < nums && sync_potential_nums < nums)
				/* will try to do sync->async */
				break;
			else
			{
				/* Added by chen_jingwen for #207866 at 2024/10/8 */
				UXSQLclear(res);
				res = NULL;
				return;  /* no need switch*/
			}
			break;
		case 2:
			confinfo = parser_info_synchronous_standby_names(sync_names_record);
			if (confinfo == 0)
			{
				strcpy(node_names[0], "*");
				/* 若有'*'配置,则当前存活节点 >= nums_record 都可以尝试进行切换 */
				res = UXSQLexec(local_conn, "SELECT count(*) FROM ux_stat_replication;");
				if (UXSQLresultStatus(res) != UXRES_TUPLES_OK)
				{
					log_error(_("unable to execute query at switch_mode: %d"), switch_mode);
					/* Added by chen_jingwen for #207866 at 2024/10/8 */
				...
				records = atoi(UXSQLgetvalue(res, 0, 0));
				if(records >= nums_record)
					break;	/* will try to do sync->async */
				else
				{
					/* Added by chen_jingwen for #207866 at 2024/10/8 */
					UXSQLclear(res);
			...
			{
				/* 若无'*'配置,则需要获取每一个节点名称 */
				if (parser_nodename_synchronous_standby_names(sync_names_record, node_names))
				{
					break;
				}
				else
				{
					log_error(_("can not switch async to sync mode"));
					return;
				}
			}
			else
			{
				log_notice(_("no need switch async to sync mode"));    /* should not happen */
				return;
			}
			break;
		default:
	...
	if (switch_mode == 1)
	{
		/* all sync standby is unreachable */
		...
		/* 
		 * Synchronous mode can be changed to asynchronous mode, and the timeout period is configurable.
		 * Default 30 seconds
		 */
		if (unreachable_standby_elapsed > max_attempts)
		{
			log_warning(_("synchronous standby node has been unreached in %ds timeout"), max_attempts);

			/* Modified by houjiaxing for #201574 at 2024/1/29 reviewer:wangbocai */
			/*sync mode transform to async mode*/
			success = alter_system_str( local_conn,
										"synchronous_standby_names",
										"''");
			if (success)
				success = ux_reload_conf(local_conn);

			if (success)
			{
				log_notice(_("switched sync mode to async mode successful"));
			}
			else
				log_warning(_("switched sync mode to async mode failed"));
		}
	}
	else if(switch_mode == 2)  /* async -> sync 模式切换 */
	{
		int match_num = 0;
		unreachable_standby_counts = 0;
		
		match_num = get_match_async_node_num(my_node_list, node_names);

		/*sync standby is reachable*/
		if (match_num >= nums_record)
		{
			...
			if (success)
			{
				log_notice(_("switched async mode to sync mode successful"));
			}
			else
				log_warning(_("switched async mode to sync mode failed"));
		}
		else
		{
			log_notice(_("no nodes with good status, not time to Recovery sync Mode."));
		}
	}
	else
		log_notice(_("switch do noting"));  /* should not happen */

	return;
}

/* 
 * parse the number of required synchronization nodes
 */
int 
parser_nums_synchronous_standby_names(char *sync_names)
{
	int i, len;
	int ret = 0;
	char *p = NULL;
	...
	strcpy(tmp, sync_names);

	if((p = strcasestr(tmp,"any")) != NULL)
	{
		for (i = 0; i <= len; i++)
		{
			if (tmp[i] == '(')
	...
	else if((p = strcasestr(sync_names,"first")) != NULL)
	{
		for (i = 0; i <= len; i++)
		{
			if (tmp[i] == '(')
...
/* BEGIN: Modify by liuxw for #205414, 2024/04/19 reviewer:hanlx */
/* 
 * parse info of sync_names, 
 * return 0: 配置中有 *, 包括: '*', 'any/first n (*)'
 *        1: 配置中无 *, 即配置为节点名称的集合
 *       -1: error, 配置为空
 */
int 
parser_info_synchronous_standby_names(char *sync_names)
{
	int len;
	int ret = -1;
...
/* END: Modify by liuxw for #205414, 2024/04/19 reviewer:hanlx */

/* 
 * parse node names of sync_names
 */
bool 
parser_nodename_synchronous_standby_names(char *sync_names, char nodename[][MAXLEN])
{
	int   i, j, len;
	char *p = NULL,*s = NULL;
	char *token = NULL;
	...
	i = 0;
	token = strtok_r(s, ",", &p);
	while (token != NULL && i < NODENUMS)
	{
		strcpy(nodename[i], token);
	...
	if (i >=  NODENUMS)
	{
		log_error(_("switch management to manage up to %d nodes"), NODENUMS);
		return false;
	}
...
check_BS(NodeInfoList *my_node_list)
{
	...
	for(mycell = my_node_list->head; mycell; mycell = mycell->next)
	{
		...
		{
			/* modifyed by houjiaxing for #189272 at 2023/07/20 reveiwer huyuanni. */
			...
			{
				TL_RET tli_ret = check_timeline(mycell->node_info->conn, &peer_node_info);

				if(tli_ret==TL_HIGH)
				{
					log_notice("more than 2 priamry nodes, local timeline > another active nodeid %d, do nothing", mycell->node_info->node_id);
					is_last_primary = false;
				}
			}
		}
	}
	...
	else if(found_other_primary>1) //the cluster has more than 2 priamry nodes, stop the whole cluster
	{
		...
		{
			log_notice("Brain split, more than 2 primary nodes were detacted. DO NOTHING");
			return DO_NOTHING;			
		}
	}
...
static TL_RET
check_timeline(UXconn *remote_conn, t_node_info *peer_node_info)
{
...
/*
 * create standby.signal (UXsinoDB 12 and later)
 * 在创建stabdby.single文件之前先检查是否已经存在该文件，
 * 存在则不创建，将current_standby_flag设置为false
 */

static bool
write_standby_signal(bool *current_standby_flag)
{
	char        standby_signal_file_path[MAXUXPATH] = "";
	FILE       *file;
	mode_t      um;

	snprintf(standby_signal_file_path, MAXUXPATH,
			"%s/%s",
			config_file_options.data_directory,
			STANDBY_SIGNAL_FILE);
	...
	if (isexist_file(standby_signal_file_path))
	{
		log_debug("dir: %s,signal file: %s",config_file_options.data_directory,standby_signal_file_path);
		*current_standby_flag = false;
		return true;
	}
	/* Set umask to 0600 */
	...
	fclose(file);
	*current_standby_flag = true;

	return true;
}

/*
 * 删除write_standby_signal函数创建的stabdby.single
 * 如果不是当前write_standby_signal创建的，则不作处理
 */
static
void delete_standby_single_file(bool current_standby_flag)
{
	UXSQLExpBufferData delete_standby_single_file_str;
	
	if (current_standby_flag == false)
		return ;
	
	initUXSQLExpBuffer(&delete_standby_single_file_str);
	appendUXSQLExpBuffer(&delete_standby_single_file_str,
		"rm -rf  %s/%s", config_file_options.data_directory, STANDBY_SIGNAL_FILE);
	ux_system(delete_standby_single_file_str.data);
	
	termUXSQLExpBuffer(&delete_standby_single_file_str);
}

static bool
isexist_file(char * filename)
{
	bool exist = false;
	if (filename == NULL)
	{
...
/* BEGIN:  Added by huyn for #184528, 2023/5/18  reviewer:houjx */
/* Construct Virtual ip connection information */
static void
get_vip_conninfo(char *conninfo, t_node_info *node_info)
{
	t_conninfo_param_list conninfo_params = T_CONNINFO_PARAM_LIST_INITIALIZER;
	bool		parse_success = false;
	char		*errmsg = NULL;
	char		*vip_conninfo = NULL;
	char		*vip_host = NULL;

	initialize_conninfo_params(&conninfo_params, false);

	parse_success = parse_conninfo_string(node_info->conninfo, &conninfo_params, &errmsg, false);

	if (parse_success == false)
	{
		log_error(_("get_vip_conninfo unable to parse provided conninfo string \"%s\""), node_info->conninfo);
		log_detail("%s", errmsg);
		free_conninfo_params(&conninfo_params);
		return ;
	}
	/* 截取virtual_ip配置项中IP地址 */
	vip_host = strtok(config_file_options.virtual_ip, "/");
	if (!vip_host)
	{
		log_warning(_("configure virtual_ip %s invaild"), config_file_options.virtual_ip);
		free_conninfo_params(&conninfo_params);
		return ;		
	}

	param_set(&conninfo_params, "host", vip_host);
	/* set some default values if not explicitly provided */
	param_set_ine(&conninfo_params, "connect_timeout", "2");

	vip_conninfo = param_list_to_string(&conninfo_params);
	if (vip_conninfo)
	{
		strncpy(conninfo, vip_conninfo, strlen(vip_conninfo));
	}

	free_conninfo_params(&conninfo_params);
	ux_free(vip_conninfo);
}

/* use vip info connect db */
static bool check_vip_connection(char *conninfo, UXconn **conn)
{
	bool conn_ok = false;
	log_debug(_("enter into check_vip_connection"));
	if (conninfo)
	{
		/* first connection */
		if (!(*conn))
		{
			*conn = establish_db_connection(conninfo, false);
			...
			if (UXSQLstatus(*conn) != CONNECTION_OK)
			{
				UXSQLfinish(*conn);
				*conn = NULL;
				log_warning(_("connection to %s failed"), conninfo);
			}
			else
			{
				log_debug(_("vip connent primary node successfully"));
				conn_ok = true;
			}
		}
		else
		{
			if (is_server_available(conninfo) == false)
			{
				log_warning(_("connection to %s lost"), conninfo);
				log_detail("\n%s", UXSQLerrorMessage(*conn));
				UXSQLfinish(*conn);
				*conn = NULL;
			}
			...
			if (UXSQLstatus(*conn) != CONNECTION_OK)
			{
				...
				*conn = establish_db_connection(conninfo, false);
				...
				if (UXSQLstatus(*conn) != CONNECTION_OK)
				{
					UXSQLfinish(*conn);
					*conn = NULL;
					log_warning(_("reconnection to %s failed"), conninfo);
				}
				else
				{
					log_debug(_("vip retry connent primary node successfully"));
					conn_ok = true;
				}
			}
			else
			{
				log_debug(_("vip old connention is vaild"));
				conn_ok = true;
			}
		}
	}

	if (conn_ok && UXSQLstatus(local_conn) != CONNECTION_OK)
	{
		UXSQLfinish(local_conn);
		local_conn = establish_db_connection(conninfo, true);
		if (UXSQLstatus(local_conn) == CONNECTION_OK)
		{
			log_debug(_("primary node conntion switch to vip conntion."));
		}
	}
	return conn_ok;
}

/* try reconnent db with vipinfo */
static void 
retry_vip_connent(char *conninfo, UXconn **conn)
{
	bool conn_ok = false;

	conn_ok = check_vip_connection(conninfo, conn);
	if (conn_ok)
	{
		if (is_primary_node_update(vip_conn, local_node_info.node_id))
		{
			log_debug(_("vip conninfo retry primary db successful."));			
		}
		else
		{
			log_notice("local node is not primary node.");
			local_node_info.node_status = NODE_STATUS_UP;
		}
	}
	else
	{
		log_debug(_("vip conninfo retry primary db fail."));
	}
}
/* END:  Added by huyn for #184528, 2023/5/18  reviewer:houjx */

/* BEGIN:  Added by huyn for #185835, 2023/5/30  reviewer:houjx */
/* check primary node deal with degraded */
static bool
is_primary_node_rejoin(char *conninfo, UXconn **conn)
{
	bool need_rejoin = false;
	if (check_vip_connection(conninfo, conn))
	{
		if (is_primary_node_update(*conn, local_node_info.node_id))
		{
			log_notice("local node is not primary node.");
			need_rejoin = true;
		}
...
/* END:  Added by huyn for #185835, 2023/5/30  reviewer:houjx */
/* BEGIN:  Added by huyn for #185853, 2023/6/1  reviewer:houjx */
/* check primary node is not change */
static bool
is_primary_node_update(UXconn *conn, int upstream_node_id)
{
	bool is_change = false;
	int primary_node_id = NODE_NOT_FOUND;
	primary_node_id = get_primary_node_id(conn);
	if (primary_node_id != NODE_NOT_FOUND && primary_node_id != UNKNOWN_NODE_ID
		 && primary_node_id != upstream_node_id)
	{
		log_notice("primary node will be change.");
		is_change = true;
	}
	return is_change;
}
/* END:  Added by huyn for #185853, 2023/6/1  reviewer:houjx */

/* add by houjiaxing for #188733 at 2023/10/16 reveiwer wangbocai */
static void
stop_the_service(void)
{
	UXSQLExpBufferData stop_service_command_str;
	int ret = -1;
	log_notice("upstream info wrong, stop db");

	if (check_network_card_status(local_conn, local_node_info.node_id) != false)
	{
		initUXSQLExpBuffer(&stop_service_command_str);
		appendUXSQLExpBuffer(&stop_service_command_str, "%s/ux_ctl -D %s stop",
							 config_file_options.ux_bindir, config_file_options.data_directory);

		ret = ux_system(stop_service_command_str.data);
		termUXSQLExpBuffer(&stop_service_command_str);
	}
	if (ret != 0)
		log_warning(_("unable to stop the db server"));
}

/* 
 * Return the number of nodes whose data very close to the primary
 */
static int
get_match_async_node_num(NodeInfoList *my_node_list, char nodename[][MAXLEN])
{
	NodeInfoListCell *mycell = NULL;
	XLogRecPtr      primary_last_wal_location = InvalidXLogRecPtr;
	...
	for (mycell = my_node_list->head; mycell; mycell = mycell->next)
	{
		/* 首节点名称不为'*'再做节点名称检查,否则对所有节点均可以做检查 */
		if (strcmp("*", nodename[0]) != 0)
		{
			for (i = 0; i < NODENUMS; i++)
			{
				if (strcmp(mycell->node_info->node_name, nodename[i]) == 0)
					break;
				else 
					continue;
			}

			/* this node(mycell) does not belong to the management of 'synchronous_standby_names' */
			if(i >= NODENUMS)
				continue;
		}
		...
		/* 跳过主节点和尚未恢复节点 */
		mycell->node_info->conn = establish_db_connection(mycell->node_info->conninfo, false);
		if (UXSQLstatus(mycell->node_info->conn) != CONNECTION_OK ||
			get_recovery_type(mycell->node_info->conn) == RECTYPE_PRIMARY)
		{
			UXSQLfinish(mycell->node_info->conn);
			mycell->node_info->conn = NULL;
			continue;
		...
		if (primary_last_wal_location != InvalidXLogRecPtr
		&& primary_last_wal_location >= last_wal_receive_lsn)
		{
			lag_bytes = (long long unsigned int)(primary_last_wal_location - last_wal_receive_lsn);
			log_notice(_("synchronous standby node's (%s) LSN is lag Primary for %lld MB  ..."), mycell->node_info->conninfo, lag_bytes / 1048576);
