...
maxpath_snprintf(char *str, const char *format,...)
{
	...
	va_start(arglist, format);
	retval = xvsnprintf(str, MAXUXPATH, format, arglist);
	va_end(arglist);
...
void
append_where_clause(UXSQLExpBufferData *where_clause, const char *format,...)
{
	va_list		arglist;
	...
	{
		appendUXSQLExpBufferStr(where_clause,
							 " WHERE ");
	}
	else
	{
...
item_list_append_format(ItemList *item_list, const char *format,...)
{
	...
	cell = (ItemListCell *) ux_malloc0(sizeof(ItemListCell));
...
_key_value_list_set(KeyValueList *item_list, bool replace, const char *key, const char *value)
{
	...
	cell = (KeyValueListCell *) ux_malloc0(sizeof(KeyValueListCell));
	...
	cell->key = ux_malloc0(keylen + 1);
	cell->value = ux_malloc0(vallen + 1);
...
check_status_list_set_format(CheckStatusList *list, const char *item, CheckStatus status, const char *details,...)
{
	...
	cell = (CheckStatusListCell *) ux_malloc0(sizeof(CheckStatusListCell));
	...
	cell->item = ux_malloc0(itemlen + 1);
	cell->details = ux_malloc0(MAXLEN);
	cell->status = status;
...
char *
escape_string(UXconn *conn, const char *string)
{
	char	   *escaped_string;
	int			error;

	escaped_string = ux_malloc0(MAXLEN);

	(void) UXSQLescapeStringConn(conn, escaped_string, string, MAXLEN, &error);
 ...
 */
void
escape_double_quotes(char *string, UXSQLExpBufferData *out)
{
	char *ptr;
	...
	for (ptr = string; *ptr; ptr++)
	{
		if (*ptr == '"')
		{
			...
			{
				appendUXSQLExpBufferChar(out, '\\');
			}
		}
		appendUXSQLExpBufferChar(out, *ptr);
	}
